 ...
 * Page class
 */
class CobrosClienteDetalleGrid extends CobrosClienteDetalle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CobrosClienteDetalleGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fcobros_cliente_detallegrid";
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_tasa_moneda") && $CurrentForm->hasValue("o_tasa_moneda") && $this->tasa_moneda->CurrentValue != $this->tasa_moneda->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_banco") && $CurrentForm->hasValue("o_banco") && $this->banco->CurrentValue != $this->banco->OldValue) {
            return false;
        }
        return true;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->tasa_moneda->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->tasa_moneda->setFormValue($val);
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['banco'] = $this->banco->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->metodo_pago->ViewValue !== null) { // Load from cache
                ...
                if ($this->metodo_pago->ViewValue == "") {
                    $this->metodo_pago->ViewValue = $Language->phrase("PleaseSelect");
                }
            ...
            if ($this->moneda->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->moneda->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
            ...
            $this->tasa_moneda->EditValue = HtmlEncode($this->tasa_moneda->CurrentValue);
            ...
            if (strval($this->tasa_moneda->EditValue) != "" && is_numeric($this->tasa_moneda->EditValue)) {
                ...
                $this->tasa_moneda->OldValue = $this->tasa_moneda->EditValue;
            }
            ...
            if ($curVal != "") {
                ...
                $this->banco->ViewValue = $this->banco->Lookup !== null && is_array($this->banco->Lookup->Options) ? $curVal : null;
            }
            if ($this->banco->ViewValue !== null) { // Load from cache
                $this->banco->EditValue = array_values($this->banco->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->banco->CurrentValue, DATATYPE_NUMBER, "");
                }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // tasa_moneda
            $this->tasa_moneda->setDbValueDef($rsnew, $this->tasa_moneda->CurrentValue, null, $this->tasa_moneda->ReadOnly);
            ...
            // Check referential integrity for master table 'cobros_cliente'
            $validMasterRecord = true;
            $masterFilter = $this->sqlMasterFilter_cobros_cliente();
            $keyValue = $rsnew['cobros_cliente'] ?? $rsold['cobros_cliente'];
            if (strval($keyValue) != "") {
            ...
            if ($validMasterRecord) {
                $rsmaster = Container("cobros_cliente")->loadRs($masterFilter)->fetch();
                $validMasterRecord = $rsmaster !== false;
            }
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "cobros_cliente", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'cobros_cliente_detalle'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_cobros_cliente();
        if ($this->cobros_cliente->getSessionValue() != "") {
        $masterFilter = str_replace("@id@", AdjustSql($this->cobros_cliente->getSessionValue(), "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if ($validMasterRecord) {
        ...
        // tasa_moneda
        $this->tasa_moneda->setDbValueDef($rsnew, $this->tasa_moneda->CurrentValue, null, false);
    ...
    public function pageDataRendering(&$header)
    {
    	...
    	foreach ($rows as $key => $value) {
    		...
    		$header .= '<a class="btn btn-primary" target="_blank" href="' . $url . '"><span class="fas fa-print"></span> Recibo de Abono # ' . $value["id"] . '</a>&nbsp;&nbsp;';
    	}
