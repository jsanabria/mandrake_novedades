...
class CobrosClienteList extends CobrosCliente
{
    ...
    public function __construct()
    {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fcobros_clientelistsrch";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcobros_clientelistsrch", $filters);
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->tipo_pago, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->pivote2, $arKeywords, $type);
        return $where;
    }
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'cobros_cliente_detalle')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("CobrosClienteDetalleList?" . Config("TABLE_SHOW_MASTER") . "=cobros_cliente&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcobros_clientelistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcobros_clientelistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fcobros_clientelist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages && $this->DetailPages["cobros_cliente_detalle"] && $this->DetailPages["cobros_cliente_detalle"]->Visible && $Security->allowList(CurrentProjectID() . 'cobros_cliente_detalle')) {
            ...
            $url = "CobrosClienteDetallePreview?t=cobros_cliente&f=" . Encrypt($sqlwrk);
            ...
            if ($Security->allowList(CurrentProjectID() . 'cobros_cliente')) {
                ...
                $links .= $link;
                $detaillnk = JsEncodeAttribute("CobrosClienteDetalleList?" . Config("TABLE_SHOW_MASTER") . "=cobros_cliente&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
    ...
    protected function loadSearchValues()
    {
        ...
        // pivote
        if (!$this->isAddOrEdit() && $this->pivote->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // pago
        if (!$this->isAddOrEdit() && $this->pago->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // tipo_pago
        if (!$this->isAddOrEdit() && $this->tipo_pago->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // pivote2
        if (!$this->isAddOrEdit() && $this->pivote2->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->cliente->AdvancedSearch->ViewValue = $this->cliente->Lookup !== null && is_array($this->cliente->Lookup->Options) ? $curVal : null;
            }
            if ($this->cliente->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`id`" . SearchString("=", $this->cliente->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $lookupFilter = function() {
                    return (CurrentPageID() == "add") ? 
            	"`id` IN 
            ...
            $this->id_documento->EditValue = HtmlEncode($this->id_documento->AdvancedSearch->SearchValue);
            ...
            $this->pago->EditValue = HtmlEncode($this->pago->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fcobros_clientelist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fcobros_clientelist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fcobros_clientelist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_cobros_cliente" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_cobros_cliente\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fcobros_clientelist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcobros_clientelistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    {
        // Example:
        $fecha = isset($_REQUEST["FechaCierre"]) ? $_REQUEST["FechaCierre"] : date("Y-m-d");
        $sql = "SELECT fecha FROM cierre_de_caja WHERE fecha = '$fecha';";
        if($row = ExecuteRow($sql)) {
        	if(VerificaFuncion("021")) {
        		$header = '<div id="xCaja"><button class="btn btn-danger" id="btnAperturaDeCaja">Aperturar Caja para el d&iacute;a ' . $fecha . '</button></div>';
        	}
        	else $header = '<div id="xCaja"><button class="btn btn-warning">Caja Cerrada para el d&iacute;a ' . $fecha . '</button></div>';
        }
        $sql = "SELECT COUNT(*) AS cantidad FROM salidas WHERE tipo_documento = 'TDCNET' AND estatus = 'NUEVO';";
        $cant = intval(ExecuteScalar($sql));
        if($cant == 0) {
       		$header .= '<br>
       		<div class="d-flex flex-column flex-sm-row">
       			<!--<button class="btn btn-primary" id="btnDetalleDeCaja">
       				Detalle de Caja
       			</button>
       			&nbsp;-->
       			<button class="btn btn-primary" id="btnResumenVentas">
       				Resumen de Ventas
       			</button>
       			&nbsp;
       			<button class="btn btn-warning" id="btnEnviarData">
       				Enviar Datos al Servidor
       			</button>
       			&nbsp;
       			<button class="btn btn-primary" id="btnCierreDeCaja">
       				Cierre de Caja
       			</button>&nbsp;';
       	   	$header .= '<input type="date" class="form-control" id="FechaCierre" name="FechaCierre" value="' . $fecha . '"> ';
       	   	$header .= '<input type="date" class="form-control" id="FechaCierre2" name="FechaCierre2" value="' . $fecha . '"></div>';
       	}
       	else {
       		$header .= '<br>
