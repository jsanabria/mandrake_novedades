...
$rs = mysqli_query($link, $sql);
$saldo_actual = "";
if($row = mysqli_fetch_array($rs)) $saldo_actual = "(Saldo actual: *** USD " . number_format($row["saldo"], 2, ".", ",") . " ***)";
...
$sql = "SELECT 
			...
			a.tasa_usd, a.monto_usd, a.saldo, c.codigo AS codigo_sanera, a.nro_recibo, IFNULL(a.cobro_cliente_reverso, 0) AS cobro_cliente_reverso   
		FROM 
			recarga a 
...
$sql = "SELECT b.nro_documento FROM cobros_cliente AS a JOIN salidas AS b ON b.id = a.id_documento WHERE a.id = $cobro_cliente_reverso";
$rs4 = mysqli_query($link, $sql);
$nto_entrega = "";
if($row4 = mysqli_fetch_array($rs4)) $nto_entrega = "NOTA de ENTREGA: " . $row4["nro_documento"];
...
if($nto_entrega == "")
	$pagos_con = "\nTipo de Pago: " . $row2["valor2"] . " - Ref. # " . $row["nro"];
else 
	$pagos_con = "\nTipo de Pago: " . $row2["valor2"] . " - $nto_entrega";
$pagos_con .= "\nMonto Recibido: " . $row["moneda"] . " " . number_format($row["monto_moneda"], 2, ".", ",");
$pagos_con .= " -- Tasa: " . number_format($row["tasa_moneda"], 2, ".", ",");
$pagos_con .= "\nMonto Bs.: " . number_format($row["monto_bs"], 2, ".", ",");
$pagos_con .= "-- Tasa USD: " . number_format($row["tasa_usd"], 2, ".", ",");
$pagos_con .= "\nMonto USD: " . number_format($row["monto_usd"], 2, ".", ",");
// $saldo = "Saldo a la fecha: USD " . number_format($row["saldo"], 2, ".", ",") . " " . $saldo_actual;
$saldo = $saldo_actual;
...
$pdf->SetFont('ARIAL','B',10);
$pdf->Cell(5,5);
//$pdf->MultiCell(190,5,utf8_decode($address),0,'L');
$pdf->Cell(95,6,$ci_rif,0,0,'L');
$pdf->SetFont('ARIAL','B',10);
//$pdf->Ln();
//$pdf->Cell(200,5,"EMAIL: " . utf8_decode($email),0,0,'R');
//$pdf->Cell(100,5,"TELEFONOS: " . utf8_decode($phone1) . ' ' . utf8_decode($phone1),0,0,'L');
/*
$pdf->Cell(100,5,"");
$pdf->SetFont('ARIAL','B',10);
$pdf->Cell(95,5,"PAGINA WEB: " . utf8_decode($webpage),0,0,'R');
*/
$pdf->Ln(6);
$pdf->SetFont('ARIAL','B',10);
$pdf->Cell(5,6);
...
$pdf->Cell(160,5,"DESCRIPCION",1,0,'C');
$pdf->Cell(35,5,"TOTAL",1,0,'R');
$pdf->Ln();

$pdf->SetFont('ARIAL','B',10);
$pdf->Cell(45,5);
$pdf->MultiCell(155,5,"$pagos_con",0,'L');
$pdf->SetFont('ARIAL','B',10);
$pdf->Cell(45,5);
$pdf->MultiCell(155,5,"$saldo",0,'L');

$pdf->Cell(5,5);
//$pdf->Cell(25,6,"1",0,0,'C');
//$pdf->Cell(135,6,$row["item"],0,0,'L');
$pdf->Cell(160,6,"",1,0,'C');
$pdf->Cell(35,6,number_format($row["monto"], 2, '.', ','),1,0,'R');
$pdf->Ln();
