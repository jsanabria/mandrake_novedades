 ...
 * Page class
 */
class ViewArticulosOnhandList extends ViewArticulosOnhand
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'view_articulos_onhand';

    // Page object name
    public $PageObjName = "ViewArticulosOnhandList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fview_articulos_onhandlist";
    ...
    public function __construct()
    {
        ...
        // Table object (view_articulos_onhand)
        if (!isset($GLOBALS["view_articulos_onhand"]) || get_class($GLOBALS["view_articulos_onhand"]) == PROJECT_NAMESPACE . "view_articulos_onhand") {
            $GLOBALS["view_articulos_onhand"] = &$this;
        }
        ...
        $this->AddUrl = "ViewArticulosOnhandAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ViewArticulosOnhandDelete";
        $this->MultiUpdateUrl = "ViewArticulosOnhandUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_articulos_onhand');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fview_articulos_onhandlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("view_articulos_onhand"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->fabricante_nombre->setVisibility();
        $this->articulo->setVisibility();
        $this->codigo_ims->setVisibility();
        $this->cantidad_real->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->fabricante_nombre->AdvancedSearch->toJson(), ","); // Field fabricante_nombre
        ...
        $filterList = Concat($filterList, $this->cantidad_real->AdvancedSearch->toJson(), ","); // Field cantidad_real
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fview_articulos_onhandlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field fabricante_nombre
        $this->fabricante_nombre->AdvancedSearch->SearchValue = @$filter["x_fabricante_nombre"];
        $this->fabricante_nombre->AdvancedSearch->SearchOperator = @$filter["z_fabricante_nombre"];
        $this->fabricante_nombre->AdvancedSearch->SearchCondition = @$filter["v_fabricante_nombre"];
        $this->fabricante_nombre->AdvancedSearch->SearchValue2 = @$filter["y_fabricante_nombre"];
        $this->fabricante_nombre->AdvancedSearch->SearchOperator2 = @$filter["w_fabricante_nombre"];
        $this->fabricante_nombre->AdvancedSearch->save();
        ...
        // Field cantidad_real
        $this->cantidad_real->AdvancedSearch->SearchValue = @$filter["x_cantidad_real"];
        $this->cantidad_real->AdvancedSearch->SearchOperator = @$filter["z_cantidad_real"];
        $this->cantidad_real->AdvancedSearch->SearchCondition = @$filter["v_cantidad_real"];
        $this->cantidad_real->AdvancedSearch->SearchValue2 = @$filter["y_cantidad_real"];
        $this->cantidad_real->AdvancedSearch->SearchOperator2 = @$filter["w_cantidad_real"];
        $this->cantidad_real->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->fabricante_nombre, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->fabricante_nombre); // fabricante_nombre
            $this->updateSort($this->articulo); // articulo
            $this->updateSort($this->codigo_ims); // codigo_ims
            $this->updateSort($this->cantidad_real); // cantidad_real
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->fabricante_nombre->setSort("");
                $this->articulo->setSort("");
                $this->codigo_ims->setSort("");
                $this->cantidad_real->setSort("");
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_articulos_onhandlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_articulos_onhandlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fview_articulos_onhandlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function newRow()
    {
        ...
        $row['fabricante_nombre'] = null;
        $row['articulo'] = null;
        $row['codigo_ims'] = null;
        $row['cantidad_real'] = null;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        // Convert decimal values if posted back
        if ($this->cantidad_real->FormValue == $this->cantidad_real->CurrentValue && is_numeric(ConvertToFloatString($this->cantidad_real->CurrentValue))) {
            $this->cantidad_real->CurrentValue = ConvertToFloatString($this->cantidad_real->CurrentValue);
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fview_articulos_onhandlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fview_articulos_onhandlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fview_articulos_onhandlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_view_articulos_onhand" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_view_articulos_onhand\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fview_articulos_onhandlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fview_articulos_onhandlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
