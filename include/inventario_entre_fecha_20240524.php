...
$out .= '<h4>Desde: ' . $fecha_desde . ' Hasta: ' . $fecha_hasta . '&nbsp; &nbsp; <a href="reportes/inventario_entre_fechas.php?fecha=' . $fecha_hasta . '" target="_blank" class="btn btn-primary">Imprimir</a></h4>';
	...
	$unidades = 0;
  	while($row = mysqli_fetch_array($rs)) {
		...
		$salidas = $row["salidas"];
		$existencia = $row["existencia"]==0 ? $devoluciones+$entradas-$salidas : $row["devoluciones"]+$row["existencia"];
		$unidades += $row["existencia"]==0 ? $devoluciones+$entradas-$salidas : $row["devoluciones"]+$row["existencia"];
