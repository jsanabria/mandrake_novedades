...
if($row = mysqli_fetch_array($rs)) {
	$saldo_actual = "(Saldo actual: *** EURO " . number_format($row["saldo"], 2, ".", ",") . " ***)";
	$saldo = $row["saldo"];
}
...
$pdf->SetFont('ARIAL','B',10);
$pdf->Cell(40,6,"B" . str_pad($nro_recibo, 7, "0", STR_PAD_LEFT),1,0,'L');
...
$pdf->Cell(25,5,"Monto EURO",1,0,'R');
$pdf->Cell(25,5,"Saldo EURO",1,0,'R');
...
while($row = mysqli_fetch_array($rs)) {
	...
	$pdf->Cell(15,5,($row["moneda"] == "USD" ? "EURO" : $row["moneda"]),0,0,'C');
