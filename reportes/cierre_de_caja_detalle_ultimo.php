	...
	function EndReport($x_arrMP, $x_RSM, $x_grandTotalBs, $x_grandTotalUsd, $x_i, $xnotas, $fecha)
	{
		...
		$sql = "SELECT 
				...
				FROM 
					salidas AS c 
				WHERE 
					c.fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND c.estatus = 'PROCESADO' AND c.tipo_documento = 'TDCNET';";
...
/////////////////////////////////////
		...
		if(intval($row["cantidad"]) > 0) {
			$sql = "SELECT 
					...
					WHERE 
						a.tipo_documento = 'TDCASA' AND fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59';"; 
			$rs = mysqli_query($link, $sql);
			$row = mysqli_fetch_array($rs);
...
$sql = "SELECT c.id, a.id,  
		...
		WHERE 
			c.fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND c.tipo_documento = 'TDCNET' -- AND c.estatus = 'PROCESADO' 
		GROUP BY 
...
$sql = "SELECT 
		...
		WHERE 
			a.fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' 
			AND (a.monto_usd <= 0 OR a.reverso = 'S') AND a.metodo_pago IN ('NC', 'ND') 
