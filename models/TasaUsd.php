...
class TasaUsd extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // id
        $this->id = new DbField('tasa_usd', 'tasa_usd', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // moneda
        $this->moneda = new DbField('tasa_usd', 'tasa_usd', 'x_moneda', 'moneda', '`moneda`', '`moneda`', 200, 10, -1, false, '`moneda`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        // tasa
        $this->tasa = new DbField('tasa_usd', 'tasa_usd', 'x_tasa', 'tasa', '`tasa`', '`tasa`', 131, 14, -1, false, '`tasa`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->tasa->Required = true; // Required field
        $this->tasa->Sortable = true; // Allow sort
        $this->tasa->DefaultDecimalPrecision = 2; // Default decimal precision
        $this->tasa->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->tasa->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->tasa->Param, "CustomMsg");
        $this->Fields['tasa'] = &$this->tasa;

        // fecha
        $this->fecha = new DbField('tasa_usd', 'tasa_usd', 'x_fecha', 'fecha', '`fecha`', CastDateFieldForLike("`fecha`", 7, "DB"), 133, 10, 7, false, '`fecha`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // hora
        $this->hora = new DbField('tasa_usd', 'tasa_usd', 'x_hora', 'hora', '`hora`', CastDateFieldForLike("`hora`", 4, "DB"), 134, 10, 4, false, '`hora`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        $this->hora->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->hora->Param, "CustomMsg");
    ...
    {
        global $Language;
        if ($pageName == "TasaUsdView") {
            return $Language->phrase("View");
        } elseif ($pageName == "TasaUsdEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "TasaUsdAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TasaUsdView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("TasaUsdView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "TasaUsdAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "TasaUsdAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("TasaUsdEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TasaUsdAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("TasaUsdDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        if (strval($this->tasa->EditValue) != "" && is_numeric($this->tasa->EditValue)) {
            $this->tasa->EditValue = FormatNumber($this->tasa->EditValue, -2, -1, -2, -1);
        }
    ...
    public function rowInserted($rsold, &$rsnew)
    {
        ...
        if($sw) {
            foreach ($dbarray as $key => $value) {
            	if($value != "") {
                	$sql = "INSERT INTO $value.tasa_usd
                			(id, moneda, tasa, fecha, hora)
                		VALUES
                			(NULL, '" . $rsnew["moneda"] . "', " . floatval($rsnew["tasa"]) . ", NOW(), NOW())";
                	Execute($sql);
                }
            }
        }
