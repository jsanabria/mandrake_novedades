	...
	function EndReport($x_arrMP, $x_RSM, $x_grandTotalBs, $x_grandTotalUsd, $x_i, $xnotas, $fecha, $fecha2)
	{
		...
		$sql = "SELECT 
				...
				WHERE 
					...
					AND c.tipo_documento = 'TDCNET' " . str_replace("xy.", "c.", $where);  
		$rs = mysqli_query($link, $sql) or die(mysqli_error());
...
/////////////////
		$sql = "SELECT 
				...
				WHERE 
					...
					AND b.metodo_pago = 'RC' " . str_replace("xy.", "a.", $where) . "
...
/////////////////////////////////////
		...
		if(intval($row["cantidad"]) > 0) {
			$sql = "SELECT 
					...
					WHERE 
						a.tipo_documento = 'TDCASA' AND fecha BETWEEN '$fecha 00:00:00' AND '$fecha2 23:59:59';"; 
			$rs = mysqli_query($link, $sql);
			$row = mysqli_fetch_array($rs);
		...
		$sql = "SELECT 
				...
				FROM 
					...
					FROM 
						...
						AND a.metodo_pago IN ('NC', 'ND')) AS a;"; 
		$rs = mysqli_query($link, $sql);
		$row = mysqli_fetch_array($rs); 
		if(intval($row["cantidad"]) > 0) {
			$sql = "SELECT 
...
//////////////////////////// REVERSOS /////////////////////////////////////
		...
		if(intval($row["cantidad"]) > 0) {
			$sql = "SELECT 
						c.nombre AS cliente, DATE_FORMAT(a.fecha, '%d/%m/%Y') AS fecha, 
			...
			$this->Ln();
			$this->Cell(10, 5);
			$this->Cell(40, 5, "Documento Reversado", 1, 0, 'L');
			$this->Cell(80, 5, "Cliente", 1, 0, 'L');
			$this->Cell(20, 5, "Fecha", 1, 0, 'L');
			$this->Cell(20, 5, "Moneda", 1, 0, 'R');
			...
			while ($row = mysqli_fetch_array($rs)) { 
				...
				$this->Cell(10, 4);
				$this->Cell(40, 4, str_pad($nrbo, 7, "0", STR_PAD_LEFT), 0, 0, 'L');
				$this->Cell(80, 4, $row["cliente"], 0, 0, 'L');
				$this->Cell(20, 4, $row["fecha"], 0, 0, 'L');
				$this->Cell(20, 4, $row["moneda"], 0, 0, 'R');
				$this->Cell(20, 4, $row["monto_moneda"], 0, 0, 'R');
				$cant++;
				$this->Ln();
			}
			$this->Cell(10, 5);
...
$sql = "SELECT c.id, a.id,  
		...
		WHERE 
			c.fecha BETWEEN '$fecha 00:00:00' AND '$fecha2 23:59:59' 
...
$sql = "SELECT 
		...
		FROM 
			...
			LEFT OUTER JOIN cliente AS d ON d.id = a.cliente 
		WHERE 
			a.fecha BETWEEN '$fecha 00:00:00' AND '$fecha2 23:59:59' AND c.estatus = 'PROCESADO' AND tipo_documento = 'TDCNET' 
			AND b.metodo_pago = 'RC' " . str_replace("xy.", "a.", $where) . ";"; 
...
$sql = "SELECT 
		...
		WHERE 
			...
			AND b.fecha BETWEEN '$fecha 00:00:00' AND '$fecha2 23:59:59' 
			AND c.estatus = 'PROCESADO' " . str_replace("xy.", "b.", $where) . "
		UNION ALL 
		SELECT 
			'RECIBO' AS TIPO, 
			a.metodo_pago, 
