...
while($row = mysqli_fetch_array($rs))
{
	...
	if(strlen($row["articulo"]) < 42) 
		$pdf->Cell(65, 3, trim($row["articulo"]), 0, 0, 'L');
	else 
		$pdf->Cell(65, 3, substr(trim($row["articulo"]), 0, 42), 0, 0, 'L');
	...
	if(strlen($row["articulo"]) >= 42) {
		$pdf->Ln();
		$pdf->Cell(30, 4);
		$pdf->MultiCell(65, 3, substr(trim($row["articulo"]), 42, strlen($row["articulo"])), 0, 'L');
	}
	else $pdf->Ln();
