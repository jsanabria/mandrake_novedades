...
class Alicuota extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // id
        $this->id = new DbField('alicuota', 'alicuota', 'x_id', 'id', '`id`', '`id`', 19, 10, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // codigo
        $this->codigo = new DbField('alicuota', 'alicuota', 'x_codigo', 'codigo', '`codigo`', '`codigo`', 200, 3, -1, false, '`codigo`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // nombre
        $this->nombre = new DbField('alicuota', 'alicuota', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, 50, -1, false, '`nombre`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // alicuota
        $this->alicuota = new DbField('alicuota', 'alicuota', 'x_alicuota', 'alicuota', '`alicuota`', '`alicuota`', 131, 6, -1, false, '`alicuota`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // fecha
        $this->fecha = new DbField('alicuota', 'alicuota', 'x_fecha', 'fecha', '`fecha`', CastDateFieldForLike("`fecha`", 7, "DB"), 133, 10, 7, false, '`fecha`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // activo
        $this->activo = new DbField('alicuota', 'alicuota', 'x_activo', 'activo', '`activo`', '`activo`', 202, 1, -1, false, '`activo`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        $this->activo->Lookup = new Lookup('activo', 'alicuota', false, '', ["","","",""], [], [], [], [], [], [], '', '');
    ...
    {
        global $Language;
        if ($pageName == "AlicuotaView") {
            return $Language->phrase("View");
        } elseif ($pageName == "AlicuotaEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "AlicuotaAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AlicuotaView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("AlicuotaView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "AlicuotaAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "AlicuotaAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("AlicuotaEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AlicuotaAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("AlicuotaDelete", $this->getUrlParm());
    }
    ...
    public function rowUpdating($rsold, &$rsnew) {
    	...
    	if($rsold["alicuota"] != $rsnew["alicuota"]) {
    		...
    		$sql = "INSERT INTO alicuota
    				...
    				SELECT NULL, codigo, nombre, '" . $rsnew["alicuota"] . "', NOW(), 'S'
    				FROM alicuota
    				WHERE id = '" . $rsold["id"] . "';";
