...
class ViewArticulos extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // referencia
        $this->referencia = new DbField('view_articulos', 'view_articulos', 'x_referencia', 'referencia', '`referencia`', '`referencia`', 200, 50, -1, false, '`referencia`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // codigo
        $this->codigo = new DbField('view_articulos', 'view_articulos', 'x_codigo', 'codigo', '`codigo`', '`codigo`', 200, 50, -1, false, '`codigo`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // fabricante
        $this->fabricante = new DbField('view_articulos', 'view_articulos', 'x_fabricante', 'fabricante', '`fabricante`', '`fabricante`', 19, 10, -1, false, '`fabricante`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        // nombre
        $this->nombre = new DbField('view_articulos', 'view_articulos', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, 100, -1, false, '`nombre`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // cantidad_en_mano
        $this->cantidad_en_mano = new DbField('view_articulos', 'view_articulos', 'x_cantidad_en_mano', 'cantidad_en_mano', '`cantidad_en_mano`', '`cantidad_en_mano`', 131, 9, -1, false, '`cantidad_en_mano`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // ultimo_costo
        $this->ultimo_costo = new DbField('view_articulos', 'view_articulos', 'x_ultimo_costo', 'ultimo_costo', '`ultimo_costo`', '`ultimo_costo`', 131, 14, -1, false, '`ultimo_costo`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // precio
        $this->precio = new DbField('view_articulos', 'view_articulos', 'x_precio', 'precio', '`precio`', '`precio`', 131, 14, -1, false, '`precio`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        global $Language;
        if ($pageName == "ViewArticulosView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ViewArticulosEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ViewArticulosAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "ViewArticulosView";
            case Config("API_ADD_ACTION"):
                return "ViewArticulosAdd";
            case Config("API_EDIT_ACTION"):
                return "ViewArticulosEdit";
            case Config("API_DELETE_ACTION"):
                return "ViewArticulosDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewArticulosView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ViewArticulosView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ViewArticulosAdd?" . $this->getUrlParm($parm);
        } else {
    ...
    {
        $url = $this->keyUrl("ViewArticulosEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewArticulosAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ViewArticulosDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function rowUpdated($rsold, &$rsnew)
    {
        ...
        ////***** Consulto si la Tienda es Principal y si lo es, esta actualización va con las demás tiendas ****////
        /* 
        ...
        fclose($fp); 
        if($sw) {
            // $sql = "SELECT codigo_ims FROM articulo WHERE id = $articulo;";
            // $referencia = ExecuteScalar($sql);
            $referencia = $rsold["referencia"];
            foreach ($dbarray as $key => $value) {
            	if($value != "") {
    	            $sql = "SELECT id FROM $value.articulo WHERE codigo_ims = '$referencia';";
    	            if($rowref = ExecuteRow($sql)) {
    	            	// $articulo = ExecuteScalar($sql);
    	            	$articulo = $rowref["id"];
    	            	$sql = "SELECT id FROM $value.tarifa WHERE patron = 'S';";
    	            	$tarifa = ExecuteScalar($sql);
    	            	$sql = "UPDATE $value.articulo SET ultimo_costo = $costo, precio = $precio WHERE id = $articulo;";
    	            	Execute($sql);
    	            	$sql = "SELECT articulo FROM $value.tarifa_articulo WHERE tarifa = $tarifa AND articulo = $articulo;";
    	            	if($row = ExecuteRow($sql)) {
    	                	$sql = "UPDATE $value.tarifa_articulo SET precio = $precio WHERE tarifa = $tarifa AND articulo = $articulo;";
    	                	Execute($sql);
    	                }
    	                else {
    	                	$sql = "INSERT INTO $value.tarifa_articulo
    	                            	(id, tarifa, fabricante, articulo, precio)
    	                            SELECT
    	                            	NULL id, $tarifa AS tarifa, fabricante, id, $precio AS precio
    	                            FROM $value.articulo WHERE id = $articulo;";
    	                    Execute($sql);
    	                }
    	            }
                }
