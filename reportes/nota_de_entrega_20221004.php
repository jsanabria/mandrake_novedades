...
$sql = "SELECT 
			...
			cliente, nro_documento, tipo_documento, estatus, cliente, username, tasa_dia   
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->SetFont('Arial','B',7);
		$this->Cell(65, 3, "FACTURA",0,0,'C');
		$this->SetFont('Arial','',7);
		$this->Ln(6);

		$this->Cell(30, 3, "FACTURA",0,0,'L');
		$this->Cell(35, 3, $GLOBALS["invoice"],0,0,'R');
		$this->Ln();

		$this->Cell(30, 3, "FECHA: " . $GLOBALS["fecha"],0,0,'L');
		$this->Cell(35, 3, "HORA: " . $GLOBALS["hora"],0,0,'R');
		$this->Ln();
	...
	function EndReport($id_invoice)
	{
		...
		$this->Cell(30, 3, "SUBSTL:", 0, 0, 'L');
		$this->Cell(35, 3, number_format(((floatval($row["exento"])+floatval($row["gravado"]))/(1+$alicuota/100)) * floatval($tasa_dia), 2, ",", "."), 0, 0, 'R');
		...
		*/
		...
		$this->Cell(15, 3, number_format(((floatval($row["exento"])+floatval($row["gravado"]))/(1+$alicuota/100)) * floatval($tasa_dia), 2, ",", "."), 0, 0, 'R');
		$this->Cell(35, 3, number_format(((floatval($row["exento"])+floatval($row["gravado"]))-(floatval($row["exento"])+floatval($row["gravado"]))/(1+$alicuota/100)) * floatval($tasa_dia), 2, ",", "."), 0, 0, 'R');
		...
		*/
		...
		$this->Cell(35, 3, "Bs. " . number_format((floatval($row["total"])) * floatval($tasa_dia), 2, ",", "."), 0, 0, 'R');
		$this->SetFont('Arial','',7);
		...
		while($row = mysqli_fetch_array($rs))
		{
			...
			$this->Cell(30, 3, substr($row2["valor2"], 0, 11), 0, 0, 'L');
			// $this->Cell(20, 3, $row["referencia"], 0, 0, 'L');

			if($row["moneda"] != "Bs.")
				$this->Cell(35, 3, "Bs. " . (floatval($row["monto_moneda"])) * $tasa_dia, 0, 0, 'R');
			else
				$this->Cell(35, 3, $row["moneda"] . " " . $row["monto_moneda"], 0, 0, 'R');
		...
		$this->Cell(30, 3, "TASA DIA Bs. " . number_format(floatval($tasa_dia), 2, ",", "."), 0, 0, 'L');
		$this->Cell(35, 3, "USD " . number_format($total, 2, ",", "."), 0, 0, 'R');
		$this->Ln();
		...
		$this->Cell(35, 3, "Z1F00" . $GLOBALS["invoice"], 0, 0, 'R');
		$this->SetFont('Arial','',7);
		$this->Ln(10);
...
{
	$pdf->SetFont('Arial', '',7);
	$pdf->Cell(20, 3, "COD: " . $row["codigo_ims"] . ". " . round($row["cantidad"], 0) . " x Bs. " . number_format($row["precio_unidad"], 2, ",", "."), 0, 0, 'L');
	$pdf->Ln();
	$pdf->Cell(40, 3, substr($row["articulo"], 0, 32), 0, 0, 'L');
