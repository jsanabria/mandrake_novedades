...
class ArticuloUnidadMedidaList extends ArticuloUnidadMedida
{
    ...
    // Table name
    public $TableName = 'articulo_unidad_medida';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["articulo_unidad_medida"]) || get_class($GLOBALS["articulo_unidad_medida"]) == PROJECT_NAMESPACE . "articulo_unidad_medida") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option farticulo_unidad_medidalistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("articulo_unidad_medida"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "articulo") {
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_unidad_medida") && $CurrentForm->hasValue("o_unidad_medida") && $this->unidad_medida->CurrentValue != $this->unidad_medida->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->unidad_medida->clearErrorMessage();
    }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "farticulo_unidad_medidalistsrch", $filters);
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->unidad_medida, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"farticulo_unidad_medidalistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"farticulo_unidad_medidalistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.farticulo_unidad_medidalist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->unidad_medida->CurrentValue = null;
        $this->unidad_medida->OldValue = $this->unidad_medida->CurrentValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['unidad_medida'] = $this->unidad_medida->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->unidad_medida->ViewCustomAttributes = "";

            // unidad_medida
            $this->unidad_medida->LinkCustomAttributes = "";
            $this->unidad_medida->HrefValue = "";
            $this->unidad_medida->TooltipValue = "";
        } elseif ($this->RowType == ROWTYPE_ADD) {
            // unidad_medida
            $this->unidad_medida->EditAttrs["class"] = "form-control";
            $this->unidad_medida->EditCustomAttributes = "";
            $curVal = trim(strval($this->unidad_medida->CurrentValue));
            if ($curVal != "") {
                $this->unidad_medida->ViewValue = $this->unidad_medida->lookupCacheOption($curVal);
            } else {
                $this->unidad_medida->ViewValue = $this->unidad_medida->Lookup !== null && is_array($this->unidad_medida->Lookup->Options) ? $curVal : null;
            }
            if ($this->unidad_medida->ViewValue !== null) { // Load from cache
                $this->unidad_medida->EditValue = array_values($this->unidad_medida->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`codigo`" . SearchString("=", $this->unidad_medida->CurrentValue, DATATYPE_STRING, "");
                }
    ...
    protected function validateForm()
    {
        ...
        if ($this->unidad_medida->Required) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // unidad_medida
            $this->unidad_medida->setDbValueDef($rsnew, $this->unidad_medida->CurrentValue, null, $this->unidad_medida->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // unidad_medida
        $this->unidad_medida->setDbValueDef($rsnew, $this->unidad_medida->CurrentValue, null, false);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.farticulo_unidad_medidalist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.farticulo_unidad_medidalist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.farticulo_unidad_medidalist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_articulo_unidad_medida" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_articulo_unidad_medida\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.farticulo_unidad_medidalist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"farticulo_unidad_medidalistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "articulo") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $articulo;
                    $articulo->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
