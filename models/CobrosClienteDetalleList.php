...
class CobrosClienteDetalleList extends CobrosClienteDetalle
{
    ...
    // Table name
    public $TableName = 'cobros_cliente_detalle';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["cobros_cliente_detalle"]) || get_class($GLOBALS["cobros_cliente_detalle"]) == PROJECT_NAMESPACE . "cobros_cliente_detalle") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fcobros_cliente_detallelistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("cobros_cliente_detalle"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "cobros_cliente") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcobros_cliente_detallelistsrch", $filters);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcobros_cliente_detallelistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcobros_cliente_detallelistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fcobros_cliente_detallelist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        // Convert decimal values if posted back
        if ($this->tasa_moneda->FormValue == $this->tasa_moneda->CurrentValue && is_numeric(ConvertToFloatString($this->tasa_moneda->CurrentValue))) {
            $this->tasa_moneda->CurrentValue = ConvertToFloatString($this->tasa_moneda->CurrentValue);
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fcobros_cliente_detallelist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fcobros_cliente_detallelist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fcobros_cliente_detallelist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_cobros_cliente_detalle" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_cobros_cliente_detalle\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fcobros_cliente_detallelist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcobros_cliente_detallelistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "cobros_cliente") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $cobros_cliente;
                    $cobros_cliente->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "cobros_cliente") {
                ...
                if (($parm = Get("fk_id", Get("cobros_cliente"))) !== null) {
                    ...
                    $this->cobros_cliente->setQueryStringValue($masterTbl->id->QueryStringValue);
            ...
            if ($masterTblVar == "cobros_cliente") {
                ...
                if (($parm = Post("fk_id", Post("cobros_cliente"))) !== null) {
                    ...
                    $this->cobros_cliente->setFormValue($masterTbl->id->FormValue);
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "cobros_cliente") {
                if ($this->cobros_cliente->CurrentValue == "") {
                    $this->cobros_cliente->setSessionValue("");
                }
            }
        }
    ...
    public function pageDataRendering(&$header)
    {
        ...
        //$header = "your header";
        $id = $_GET["fk_id"];
        $sql = "SELECT id_documento FROM cobros_cliente WHERE id = $id;";
        $id_documento = ExecuteScalar($sql);
        $sql = "SELECT tipo_documento FROM salidas WHERE id = $id_documento;";
        $tipo = ExecuteScalar($sql);
       	$url = "SalidasView/$id_documento?showdetail=entradas_salidas&tipo=$tipo";
        ...
        $id = isset($_GET["fk_id"]) ? intval($_GET["fk_id"]) : 0;
       	$sql = "SELECT id_documento FROM cobros_cliente WHERE id = $id;";
       	$id_documento = ExecuteScalar($sql);
       	$sql = "SELECT tipo_documento, estatus FROM salidas WHERE id = $id_documento;";
       	$row = ExecuteRow($sql);
       	$tipo_documento = $row["tipo_documento"];
       	$estatus = $row["estatus"];
    	if($tipo_documento == "TDCNET") {
    		if($estatus == "PROCESADO") {				
    			$url = "reportes/nota_de_entrega.php?&id=$id_documento&tipo=$tipo_documento&CurrentUserName=" . CurrentUserName() . "";
    			$header .= '<a class="btn btn-primary" href="' . $url . '" target="_blank" ><span class="fas fa-print"></span> Nota de Entrega</a>&nbsp;&nbsp;';
    			$url = "reportes/nota_de_entrega_ticket.php?&id=$id_documento&tipo=$tipo_documento&CurrentUserName=" . CurrentUserName() . "";
    			$header .= ' <a class="btn btn-primary" href="' . $url . '" target="_blank" ><span class="fas fa-map"></span> Ticket Nota de Entrega</a>&nbsp;&nbsp;';
    		}
    	}   	
    	$sql = "SELECT c.id
    	...
    	foreach ($rows as $key => $value) {
    		...
    		$header .= '<a class="btn btn-primary" target="_blank" href="' . $url . '"><span class="fas fa-print"></span> Recibo de Abono en Bs # ' . $value["id"] . '</a>&nbsp;&nbsp;';
    	}
    	$sql = "SELECT c.id
    	...
    	foreach ($rows as $key => $value) {
    		...
    		$header .= '<a class="btn btn-primary" target="_blank" href="' . $url . '"><span class="fas fa-print"></span> Recibo de Abono en USD # ' . $value["id"] . '</a>&nbsp;&nbsp;';
    	}
    	if($tipo_documento != "TDCPDV") {
    		$header .= '<a class="btn btn-primary" id="btnNuevo" href="SalidasAdd?showdetail=entradas_salidas&tipo=' . $tipo . '"><span class="fas fa-plus"></span> Nuevo Documento</a>&nbsp;&nbsp;';
    	}

       	////////////////////////////
        $id = isset($_GET["fk_id"]) ? intval($_GET["fk_id"]) : 0;
        if($id > 0) {
        	$sql = "SELECT cliente FROM cobros_cliente WHERE id = $id;";
        	$cliente = ExecuteScalar($sql);
        	$sql = "SELECT id, saldo FROM recarga WHERE cliente = $cliente ORDER BY id DESC LIMIT 0, 1;";
        	if($row = ExecuteRow($sql)) {
        		$saldo = floatval($row["saldo"]);
        		if($saldo > 0.00) {
        			$tipo_name = '&nbsp; <span class="text-success"><strong>Saldo: $ ' . number_format($saldo, 2, ".", ",") . '</strong></span>';
        		}
        		else { 
        			$tipo_name = '&nbsp; <span class="text-danger"><strong>Saldo: $ ' . number_format($saldo, 2, ".", ",") . '</strong></span>';
        		}
        		$header .= $tipo_name;
        	}
        }
