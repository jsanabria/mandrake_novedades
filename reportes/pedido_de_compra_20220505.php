...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(20, 6, "UNIDA MED", 1, 0, 'C');
		$this->Cell(20, 6, "CANTIDAD", 1, 0, 'R');
		$this->Cell(25, 6, "COSTO UNIDAD.", 1, 0, 'R');
...
$sql = "SELECT 
			...
			a.costo_unidad, 
			a.costo, a.lote, date_format(a.fecha_vencimiento, '%d/%m/%Y') AS vencimiento 
		FROM 
...
while($row = mysqli_fetch_array($rs))
{
	...
	if(strlen($row["articulo"]) < 35) 
		...
		$pdf->Cell(80, 4, substr($row["articulo"], 0, 35), 0, 0, 'L');
	$pdf->Cell(20, 4, $row["unidad_medida"], 0, 0, 'C');
	$pdf->Cell(20, 4, $row["cantidad"], 0, 0, 'R');
	...
	if(strlen($row["articulo"]) >= 55) {
		$pdf->Ln();
		$pdf->Cell(30, 4);
		$pdf->MultiCell(130, 4, substr($row["articulo"], 55, strlen($row["articulo"])), 0, 'L');
	}
	else $pdf->Ln();
