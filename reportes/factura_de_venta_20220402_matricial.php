...
if(($monto_usd==0 or $tasa_dia==0 or $tasa_dia==1)) { 
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->SetFont('Arial','B',10);
		$this->Cell(10, 6);
		$this->Cell(20, 6, "REF. #", 1, 0, 'L');
		$this->Cell(80, 6, "ARTICULO", 1, 0, 'L');
		$this->Cell(15, 6, "CAN.", 1, 0, 'R');
		$this->Cell(15, 6, "IVA %", 1, 0, 'R');
		$this->Cell(30, 6, "PRECIO", 1, 0, 'R');
	...
	function EndReport($id_invoice)
	{
		...
		$this->SetFont('Arial','B',10);
		$this->Cell(160, 4, $asociado . " " . "SUB-TOTAL EXENTO $moneda:", 0, 0, 'R');
		$this->SetFont('Arial','B',10);
		...
		$this->Cell(50,4, "TOTAL $moneda: " . number_format($monto_usd, 2, ".", ","), 0, 0, 'L');
		$this->Cell(50,4, "TC: " . number_format($tasa_dia, 2, ".", ","), 0, 0, 'L');
		$this->SetFont('Arial','B',10);
		$this->Cell(40,4, "SUB-TOTAL GRAVADO $moneda:", 0, 0, 'R');
		$this->SetFont('Arial','B',10);
		...
		if(floatval($alicuota) > 0) {
			...
			$this->SetFont('Arial','B',10);
			$this->Cell(160,4, "IVA $moneda:", 0, 0, 'R');
			$this->SetFont('Arial','B',10);
		}
...
$sql = "SELECT 
			...
			a.precio_unidad, 
			a.precio, 
			c.nombre AS fabricante, b.codigo_ims  
		FROM 
...
while($row = mysqli_fetch_array($rs))
{
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(10, 6);
	$pdf->Cell(20, 6, $row["codigo_ims"], 0, 0, 'L');
	if(strlen($row["articulo"]) < 34) 
		$pdf->Cell(80, 6, $row["articulo"], 0, 0, 'L');
	else 
		$pdf->Cell(80, 6, substr($row["articulo"], 0, 34), 0, 0, 'L');

	$pdf->Cell(15, 6, number_format($row["cantidad"], 0, "", ""), 0, 0, 'R');
	$pdf->Cell(15, 6, number_format($row["alicuota"], 2, ".", ","), 0, 0, 'R');
	$pdf->Cell(30, 6, number_format($row["precio_unidad"], 2, ".", ","), 0, 0, 'R');
	$pdf->Cell(30, 6, number_format($row["precio"], 2, ".", ","), 0, 0, 'R');

	if(strlen($row["articulo"]) >= 34) {
		$pdf->Ln();
		$pdf->Cell(30, 4);
		$pdf->MultiCell(80, 6, substr($row["articulo"], 34, strlen($row["articulo"])), 0, 'L');
	}
	else $pdf->Ln();
