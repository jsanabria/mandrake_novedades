...
if($tipo != "") $where = "AND d.codigo_ims IN ('$tipo')"; 
	$sql = "SELECT 
					...
					ROUND(((IFNULL(b.precio_unidad, 0)-IFNULL(b.costo_unidad, 0))/IFNULL(b.precio_unidad,0))*100, 2) AS utilidad  
				FROM 
					salidas AS a 
				...
				WHERE 
					...
					$where 
				GROUP BY d.id, CONCAT(IFNULL(d.principio_activo, ' '), ' ', IFNULL(d.presentacion, ' '), ' ', IFNULL(d.nombre_comercial, ' ')), b.costo_unidad, b.precio_unidad ORDER BY d.codigo_ims ASC;"; 
