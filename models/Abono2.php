...
/**
 * Table class for abono2
 */
class Abono2 extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'abono2';
        $this->TableName = 'abono2';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`abono2`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('abono2', 'abono2', 'x_id', 'id', '`id`', '`id`', 21, 20, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // cliente
        $this->cliente = new DbField('abono2', 'abono2', 'x_cliente', 'cliente', '`cliente`', '`cliente`', 19, 10, -1, false, '`cliente`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // fecha
        $this->fecha = new DbField('abono2', 'abono2', 'x_fecha', 'fecha', '`fecha`', CastDateFieldForLike("`fecha`", 7, "DB"), 133, 10, 7, false, '`fecha`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // nro_recibo
        $this->nro_recibo = new DbField('abono2', 'abono2', 'x_nro_recibo', 'nro_recibo', '`nro_recibo`', '`nro_recibo`', 19, 10, -1, false, '`nro_recibo`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // pago
        $this->pago = new DbField('abono2', 'abono2', 'x_pago', 'pago', '`pago`', '`pago`', 131, 14, -1, false, '`pago`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // nota
        $this->nota = new DbField('abono2', 'abono2', 'x_nota', 'nota', '`nota`', '`nota`', 200, 250, -1, false, '`nota`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // metodo_pago
        $this->metodo_pago = new DbField('abono2', 'abono2', 'x_metodo_pago', 'metodo_pago', '`metodo_pago`', '`metodo_pago`', 200, 10, -1, false, '`metodo_pago`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        // username
        $this->_username = new DbField('abono2', 'abono2', 'x__username', 'username', '`username`', '`username`', 200, 30, -1, false, '`username`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // pivote
        $this->pivote = new DbField('abono2', 'abono2', 'x_pivote', 'pivote', '`pivote`', '`pivote`', 200, 1, -1, false, '`pivote`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // pivote2
        $this->pivote2 = new DbField('abono2', 'abono2', 'x_pivote2', 'pivote2', '`pivote2`', '`pivote2`', 200, 1, -1, false, '`pivote2`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "recarga2") {
            $detailUrl = Container("recarga2")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "Abono2List";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`abono2`";
    }
    ...
    {
        // Cascade Update detail table 'recarga2'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("recarga2")->loadRs("`abono` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("recarga2")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("recarga2")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("recarga2")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'recarga2'
        $dtlrows = Container("recarga2")->loadRs("`abono` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("recarga2")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("recarga2")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("recarga2")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("Abono2List");
    }
    ...
    {
        global $Language;
        if ($pageName == "Abono2View") {
            return $Language->phrase("View");
        } elseif ($pageName == "Abono2Edit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "Abono2Add") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("Abono2View", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("Abono2View", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "Abono2Add?" . $this->getUrlParm($parm);
        } else {
            $url = "Abono2Add";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("Abono2Edit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("Abono2Edit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("Abono2Add", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("Abono2Add", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("Abono2Delete", $this->getUrlParm());
    }
    ...
    public function rowInserted($rsold, &$rsnew)
    {
    	$arr = explode(",-,", $_POST["pagos"]);
      	foreach ($arr as $key => $value) {
      	 ...
      	 $sql = "SELECT SUM(monto_usd) AS pago FROM recarga2 WHERE abono = " . $rsnew["id"] . ";";
      	 $monto_abono = floatval(ExecuteScalar($sql));
      	 $sql = "UPDATE abono2 SET pago = $monto_abono WHERE id = " . $rsnew["id"] . ";";
      	 Execute($sql);
    }
