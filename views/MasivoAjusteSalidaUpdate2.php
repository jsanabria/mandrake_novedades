...
// Page object
$MasivoAjusteSalidaUpdate2 = &$Page;
?>
<?php
$id = $_REQUEST["id"];
					  ...
					  <table class="table table-bordered table-condensed table-striped">
							...
							$sql= "SELECT 
										...
										CONCAT(IFNULL(c.principio_activo, ''), ', ', 
											...
											d.cantidad, a.articulo AS codart, id_compra, articulo_unidad_medida    
									FROM 
						  			...
						  			<td><?php 
						  				...
						  				$sql = "SELECT 
												...
												WHERE
													...
													AND b.estatus = 'PROCESADO' AND a.articulo = '$codart' 
													AND (IFNULL(a.cantidad_movimiento, 0) + 
													(if(" . intval($row["id_compra"]) . " = a.id, 0, IFNULL(d.cantidad_movimiento, 0)))
													) > 0  
										...
										for($j=0; $j<$cntun; $j++) {
											...
											if($row["id_compra"] == $row2["id"]) {
												echo '<option value="' . $row2["id"] . '" selected="selected">' . $row2["lote"] . ", " . $row2["fecha_vencimiento"] . ", " . $row2["cantidad"] . '</option>';
												}
											else {
...
//alert(i + ": " + id + " - " + lote + " - " + cantidad + " - " + unidad);
	...
	$("#enviar").click(function() {
		...
		if(confirm("Está seguro de crear la nota de entrega?")) {
			...
			$.ajax({
				...
				success: function (response) {//resultado de la función
					//alert(response);
					window.location.href = "SalidasList?tipo=TDCNET";
				}
			});
		}
	});
