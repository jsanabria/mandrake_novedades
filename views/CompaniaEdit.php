...
loadjs.ready("head", function () {
    ...
    fcompaniaedit = currentForm = new ew.Form("fcompaniaedit", "edit");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "compania")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.compania)
        ew.vars.tables.compania = currentTable;
    fcompaniaedit.addFields([
    ...
    // Validate form
    fcompaniaedit.validate = function () {
    ...
    // Form_CustomValidate
    fcompaniaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcompaniaedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fcompaniaedit.lists.ciudad = <?= $Page->ciudad->toClientList($Page) ?>;
    fcompaniaedit.lists.agente_retencion = <?= $Page->agente_retencion->toClientList($Page) ?>;
    loadjs.done("fcompaniaedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fcompaniaedit" id="fcompaniaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="<?= $Page->ci_rif->getInputTextType() ?>" data-table="compania" data-field="x_ci_rif" name="x_ci_rif" id="x_ci_rif" size="30" maxlength="30" placeholder="<?= HtmlEncode($Page->ci_rif->getPlaceHolder()) ?>" value="<?= $Page->ci_rif->EditValue ?>"<?= $Page->ci_rif->editAttributes() ?> aria-describedby="x_ci_rif_help">
...
<input type="<?= $Page->nombre->getInputTextType() ?>" data-table="compania" data-field="x_nombre" name="x_nombre" id="x_nombre" size="30" maxlength="80" placeholder="<?= HtmlEncode($Page->nombre->getPlaceHolder()) ?>" value="<?= $Page->nombre->EditValue ?>"<?= $Page->nombre->editAttributes() ?> aria-describedby="x_nombre_help">
...
loadjs.ready(["fcompaniaedit"], function() {
    fcompaniaedit.createAutoSuggest(Object.assign({"id":"x_ciudad","forceSelect":true}, ew.vars.tables.compania.fields.ciudad.autoSuggestOptions));
});
</script>
...
<input type="<?= $Page->telefono1->getInputTextType() ?>" data-table="compania" data-field="x_telefono1" name="x_telefono1" id="x_telefono1" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->telefono1->getPlaceHolder()) ?>" value="<?= $Page->telefono1->EditValue ?>"<?= $Page->telefono1->editAttributes() ?> aria-describedby="x_telefono1_help">
...
<input type="<?= $Page->telefono2->getInputTextType() ?>" data-table="compania" data-field="x_telefono2" name="x_telefono2" id="x_telefono2" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->telefono2->getPlaceHolder()) ?>" value="<?= $Page->telefono2->EditValue ?>"<?= $Page->telefono2->editAttributes() ?> aria-describedby="x_telefono2_help">
...
<input type="<?= $Page->email1->getInputTextType() ?>" data-table="compania" data-field="x_email1" name="x_email1" id="x_email1" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->email1->getPlaceHolder()) ?>" value="<?= $Page->email1->EditValue ?>"<?= $Page->email1->editAttributes() ?> aria-describedby="x_email1_help">
...
<input type="<?= $Page->email2->getInputTextType() ?>" data-table="compania" data-field="x_email2" name="x_email2" id="x_email2" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->email2->getPlaceHolder()) ?>" value="<?= $Page->email2->EditValue ?>"<?= $Page->email2->editAttributes() ?> aria-describedby="x_email2_help">
...
<?php if ($Page->agente_retencion->Visible) { // agente_retencion ?>
    <div id="r_agente_retencion" class="form-group row">
...
<div class="input-group">
    <div class="custom-file">
        <input type="file" class="custom-file-input" title="<?= $Page->logo->title() ?>" data-table="compania" data-field="x_logo" name="x_logo" id="x_logo" lang="<?= CurrentLanguageID() ?>"<?= $Page->logo->editAttributes() ?><?= ($Page->logo->ReadOnly || $Page->logo->Disabled) ? " disabled" : "" ?> aria-describedby="x_logo_help">
