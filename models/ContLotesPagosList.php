...
class ContLotesPagosList extends ContLotesPagos
{
    ...
    // Table name
    public $TableName = 'cont_lotes_pagos';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["cont_lotes_pagos"]) || get_class($GLOBALS["cont_lotes_pagos"]) == PROJECT_NAMESPACE . "cont_lotes_pagos") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fcont_lotes_pagoslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("cont_lotes_pagos"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcont_lotes_pagoslistsrch", $filters);
    ...
    protected function setupListOptions()
    {
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'cont_lotes_pagos_detalle') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'cont_lotes_pagos_detalle')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("ContLotesPagosDetalleList?" . Config("TABLE_SHOW_MASTER") . "=cont_lotes_pagos&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cont_lotes_pagos')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=cont_lotes_pagos_detalle");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "cont_lotes_pagos_detalle";
            }
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcont_lotes_pagoslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcont_lotes_pagoslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fcont_lotes_pagoslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`cont_lotes_pago`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";

        // Column "detail_cont_lotes_pagos_detalle"
        if ($this->DetailPages && $this->DetailPages["cont_lotes_pagos_detalle"] && $this->DetailPages["cont_lotes_pagos_detalle"]->Visible && $Security->allowList(CurrentProjectID() . 'cont_lotes_pagos_detalle')) {
            $link = "";
            $option = $this->ListOptions["detail_cont_lotes_pagos_detalle"];
            $url = "ContLotesPagosDetallePreview?t=cont_lotes_pagos&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"cont_lotes_pagos_detalle\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'cont_lotes_pagos')) {
                $label = $Language->TablePhrase("cont_lotes_pagos_detalle", "TblCaption");
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"cont_lotes_pagos_detalle\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("ContLotesPagosDetalleList?" . Config("TABLE_SHOW_MASTER") . "=cont_lotes_pagos&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . $Language->TablePhrase("cont_lotes_pagos_detalle", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</a>";
            }
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cont_lotes_pagos')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=cont_lotes_pagos_detalle");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</a>";
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcont_lotes_pagoslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
