 ...
 * Page class
 */
class PuntosEdit extends Puntos
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    $this->terminate("PuntosList"); // Return to list page
                    return;
        ...
        switch ($this->CurrentAction) {
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "PuntosList") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->fecha->IsDetailKey) {
            ...
            $this->fecha->CurrentValue = UnFormatDateTime($this->fecha->CurrentValue, 14);
        }
        ...
        // Check field name 'puntos' first before field var 'x_puntos'
        $val = $CurrentForm->hasValue("puntos") ? $CurrentForm->getValue("puntos") : $CurrentForm->getValue("x_puntos");
        if (!$this->puntos->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->puntos->Visible = false; // Disable update for API request
            } else {
                $this->puntos->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->puntos->CurrentValue = $this->puntos->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->fecha->ViewValue = FormatDateTime($this->fecha->ViewValue, 14);
            ...
            $this->fecha->EditValue = HtmlEncode(FormatDateTime($this->fecha->CurrentValue, 14));
            ...
            if ($curVal != "") {
                ...
                $this->referencia->ViewValue = $this->referencia->Lookup !== null && is_array($this->referencia->Lookup->Options) ? $curVal : null;
            }
            if ($this->referencia->ViewValue !== null) { // Load from cache
                $this->referencia->EditValue = array_values($this->referencia->Lookup->Options);
                if ($this->referencia->ViewValue == "") {
                    $this->referencia->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`codigo_ims`" . SearchString("=", $this->referencia->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->referencia->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->referencia->Lookup->renderViewRow($rswrk[0]);
                ...
                $arwrk = $rswrk;
                $this->referencia->EditValue = $arwrk;
            }
            ...
            $this->puntos->EditValue = HtmlEncode($this->puntos->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if (!CheckShortEuroDate($this->fecha->FormValue)) {
        ...
        if ($this->puntos->Required) {
            if (!$this->puntos->IsDetailKey && EmptyValue($this->puntos->FormValue)) {
                $this->puntos->addErrorMessage(str_replace("%s", $this->puntos->caption(), $this->puntos->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->puntos->FormValue)) {
            $this->puntos->addErrorMessage($this->puntos->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // fecha
            $this->fecha->setDbValueDef($rsnew, UnFormatDateTime($this->fecha->CurrentValue, 14), null, $this->fecha->ReadOnly);
            ...
            // puntos
            $this->puntos->setDbValueDef($rsnew, $this->puntos->CurrentValue, null, $this->puntos->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PuntosList"), "", $this->TableVar, true);
        $pageId = "edit";
