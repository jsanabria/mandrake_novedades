...
$sql = "SELECT 
        ...
        d.descripcion AS unidad_medida, a.descuento  
      FROM 
        articulo AS a 
...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->Cell(60, 5, substr($art, 0, 30), 1, 0, 'L');
	$pre = trim($row["presentacion"]);
	$pdf->Cell(50, 5, substr($pre, 0, 30), 1, 0, 'L');
	$pdf->Cell(25, 5, number_format($row["precio"], 2, ".", ","), 1, 0, 'R');
	$pdf->Cell(15, 5, (floatval($row["descuento"]) == 0 ? '' : number_format($row["descuento"], 2, ".", ",")), 1, 0, 'R');
	$pdf->Cell(15, 5, "", 1, 0, 'C');
	$pdf->Cell(10, 5, substr($row["unidad_medida"], 0, 4), 1, 0, 'C');
	$pdf->Ln();
	$items++;

	if(strlen($art) > 30 or strlen($pre) > 30) {
		$pdf->Cell(5, 5);
		$pdf->Cell(25, 5, "", 1, 0, 'C');
		$pdf->Cell(60, 5, substr($art, 30, 30), 1, 0, 'L');
		$pdf->Cell(50, 5, substr($pre, 30, 30), 1, 0, 'L');
		$pdf->Cell(65, 5, "", 1, 0, 'C');
		$pdf->Ln();
	}
}
