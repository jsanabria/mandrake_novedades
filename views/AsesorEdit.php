...
loadjs.ready("head", function () {
    ...
    fasesoredit = currentForm = new ew.Form("fasesoredit", "edit");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "asesor")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.asesor)
        ew.vars.tables.asesor = currentTable;
    fasesoredit.addFields([
    ...
    // Form_CustomValidate
    fasesoredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fasesoredit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fasesoredit.lists.ciudad = <?= $Page->ciudad->toClientList($Page) ?>;
    fasesoredit.lists.activo = <?= $Page->activo->toClientList($Page) ?>;
    loadjs.done("fasesoredit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fasesoredit" id="fasesoredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="<?= $Page->ci_rif->getInputTextType() ?>" data-table="asesor" data-field="x_ci_rif" name="x_ci_rif" id="x_ci_rif" size="30" maxlength="30" placeholder="<?= HtmlEncode($Page->ci_rif->getPlaceHolder()) ?>" value="<?= $Page->ci_rif->EditValue ?>"<?= $Page->ci_rif->editAttributes() ?> aria-describedby="x_ci_rif_help">
...
<input type="<?= $Page->nombre->getInputTextType() ?>" data-table="asesor" data-field="x_nombre" name="x_nombre" id="x_nombre" size="30" maxlength="80" placeholder="<?= HtmlEncode($Page->nombre->getPlaceHolder()) ?>" value="<?= $Page->nombre->EditValue ?>"<?= $Page->nombre->editAttributes() ?> aria-describedby="x_nombre_help">
...
<input type="<?= $Page->telefono1->getInputTextType() ?>" data-table="asesor" data-field="x_telefono1" name="x_telefono1" id="x_telefono1" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->telefono1->getPlaceHolder()) ?>" value="<?= $Page->telefono1->EditValue ?>"<?= $Page->telefono1->editAttributes() ?> aria-describedby="x_telefono1_help">
...
<input type="<?= $Page->telefono2->getInputTextType() ?>" data-table="asesor" data-field="x_telefono2" name="x_telefono2" id="x_telefono2" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->telefono2->getPlaceHolder()) ?>" value="<?= $Page->telefono2->EditValue ?>"<?= $Page->telefono2->editAttributes() ?> aria-describedby="x_telefono2_help">
...
<input type="<?= $Page->email1->getInputTextType() ?>" data-table="asesor" data-field="x_email1" name="x_email1" id="x_email1" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->email1->getPlaceHolder()) ?>" value="<?= $Page->email1->EditValue ?>"<?= $Page->email1->editAttributes() ?> aria-describedby="x_email1_help">
...
<input type="<?= $Page->email2->getInputTextType() ?>" data-table="asesor" data-field="x_email2" name="x_email2" id="x_email2" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->email2->getPlaceHolder()) ?>" value="<?= $Page->email2->EditValue ?>"<?= $Page->email2->editAttributes() ?> aria-describedby="x_email2_help">
...
<input type="<?= $Page->web->getInputTextType() ?>" data-table="asesor" data-field="x_web" name="x_web" id="x_web" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->web->getPlaceHolder()) ?>" value="<?= $Page->web->EditValue ?>"<?= $Page->web->editAttributes() ?> aria-describedby="x_web_help">
...
<span id="el_asesor_activo">
    <select
        ...
        data-select2-id="asesor_x_activo"
        data-table="asesor"
        data-field="x_activo"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='asesor_x_activo']"),
        options = { name: "x_activo", selectId: "asesor_x_activo", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.data = ew.vars.tables.asesor.fields.activo.lookupOptions;
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.asesor.fields.activo.selectOptions);
...
loadjs.ready("load", function () {
    // Startup script
    $("#x_telefono1").mask("(9999) 999-99-99"),$("#x_telefono2").mask("(9999) 999-99-99");
});
</script>
