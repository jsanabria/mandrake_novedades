...
///////////////////////////////////
		$sql = "SELECT 
					...
					SUM(IF(metodo_pago='PF' OR metodo_pago='PC' OR metodo_pago='DV', (-1), 1)*monto_bs) AS monto_bs, 
					SUM(IF(metodo_pago='PF' OR metodo_pago='PC' OR metodo_pago='DV', (-1), 1)*monto_usd) AS monto_usd
				FROM recarga WHERE fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND (monto_usd > 0  OR (reverso = 'S' AND monto_usd < 0)) 
...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->Cell(88, 4, ($row["tipo_pago"] == "RC" ? "Egreso Cuenta Cliente " . ($row["estatus"] == "ANULADO" ? "" : number_format($row["total"], 2, ".", ",") . " - " . number_format($row["rc_monto_usd"], 2, ".", ",") . " = " . number_format($pago, 2, ".", ",")) : "Contado") .  ($row["estatus"] == "PROCESADO" ? "" : " - " . $row["estatus"]), 0, 0, 'L');
...
$sql = "SELECT 
			...
			IF(a.metodo_pago='RC', 
				'EXCEDENTE EN PAGO', 
				IF(a.metodo_pago='PF' OR a.metodo_pago='PC' OR a.metodo_pago='DV', 
			...
			a.tasa_usd, 
			a.nro_recibo, 
			IF(a.metodo_pago='PF' OR a.metodo_pago='PC' OR a.metodo_pago='DV', (-1), 1)*a.monto_bs AS monto_bs, 
			IF(a.metodo_pago='PF' OR a.metodo_pago='PC' OR a.metodo_pago='DV', (-1), 1)*a.monto_usd AS monto_usd, 
		...
		WHERE 
			a.fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' 
			AND (a.monto_usd > 0 OR a.reverso = 'S') 
		ORDER BY metodo_pago, a.nro_recibo;"; 
...
while($row = mysqli_fetch_array($rs))
{
	...
	if($row["tipo_pago"] == "PC" or $row["tipo_pago"] == "PF" or $row["tipo_pago"] == "DV" or $row["tipo_pago"] == "AN") {
		$pdf->SetFont('Arial', 'BI', 8);
	} 
	else {
		$pdf->SetFont('Arial', '', 8);
	...
	if(($row["reverso"]=="S" and floatval($row["monto_usd"])>0)) {
		...
		$pdf->Cell(40, 4, number_format(($row["tipo_pago"] == "PC" or $row["tipo_pago"] == "PF" or $row["tipo_pago"] == "DV" or $row["tipo_pago"] == "AN") ? abs($row["monto_usd"]) : floatval($row["monto_usd"]), 2, ".", ","), 0, 0, 'R');
		$pdf->Ln();
		if(($row["monto_usd"] >= 0 and $row["reverso"] == "N") or ($row["reverso"] == "S" and $row["tipo_pago"] != "PC" and $row["tipo_pago"] != "PF" and $row["tipo_pago"] != "DV" and $row["tipo_pago"] != "AN")) {
			$TotalBs += $row["monto_bs"];
			$TotalUsd += $row["monto_usd"];
...
$pdf->Ln();
$pdf->Cell(10, 6);
$pdf->Cell(35, 6, "TIPO DE PAGO", 1, 0, 'L');
$pdf->Cell(15, 6, "# REF.", 1, 0, 'L');
...
$sql = "SELECT 
		...
		WHERE 
			a.metodo_pago NOT IN ('RC', 'PF', 'PC', 'DV') 
		...
		WHERE bb.codigo = '009' AND aa.metodo_pago NOT IN ('RC', 'PF', 'PC', 'DV') 
		ORDER BY 2, 1,3;"; 
