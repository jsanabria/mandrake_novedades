...
loadjs.ready("head", function () {
    ...
    frecargalist = currentForm = new ew.Form("frecargalist", "list");
    frecargalist.formKeyCountName = '<?= $Page->FormKeyCountName ?>';
    loadjs.done("frecargalist");
});
var frecargalistsrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object for search
    frecargalistsrch = currentSearchForm = new ew.Form("frecargalistsrch");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "recarga")) ?>,
        fields = currentTable.fields;
    frecargalistsrch.addFields([
    ...
    // Set invalid fields
    $(function() {
        frecargalistsrch.setInvalid();
    });

    // Validate form
    frecargalistsrch.validate = function () {
    ...
    // Form_CustomValidate
    frecargalistsrch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frecargalistsrch.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    frecargalistsrch.lists.metodo_pago = <?= $Page->metodo_pago->toClientList($Page) ?>;
    frecargalistsrch.lists.moneda = <?= $Page->moneda->toClientList($Page) ?>;

    // Filters
    frecargalistsrch.filterList = <?= $Page->getFilterList() ?>;

    // Init search panel as collapsed
    frecargalistsrch.initSearchPanel = true;
    loadjs.done("frecargalistsrch");
});
</script>
<style>
...
<form name="frecargalistsrch" id="frecargalistsrch" class="form-inline ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>">
<div id="frecargalistsrch-search-panel" class="<?= $Page->SearchPanelClass ?>">
...
<input type="hidden" name="z_fecha" id="z_fecha" value="<=">
        ...
        <span id="el_recarga_fecha" class="ew-search-field">
<input type="<?= $Page->fecha->getInputTextType() ?>" data-table="recarga" data-field="x_fecha" data-format="7" name="x_fecha" id="x_fecha" maxlength="10" placeholder="<?= HtmlEncode($Page->fecha->getPlaceHolder()) ?>" value="<?= $Page->fecha->EditValue ?>"<?= $Page->fecha->editAttributes() ?>>
<div class="invalid-feedback"><?= $Page->fecha->getErrorMessage(false) ?></div>
<?php if (!$Page->fecha->ReadOnly && !$Page->fecha->Disabled && !isset($Page->fecha->EditAttrs["readonly"]) && !isset($Page->fecha->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["frecargalistsrch", "datetimepicker"], function() {
    ew.createDateTimePicker("frecargalistsrch", "x_fecha", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<input type="hidden" name="z_metodo_pago" id="z_metodo_pago" value="LIKE">
        ...
        <span id="el_recarga_metodo_pago" class="ew-search-field">
    <select
        id="x_metodo_pago"
        name="x_metodo_pago"
...
<input type="hidden" name="z_moneda" id="z_moneda" value="LIKE">
        ...
        <span id="el_recarga_moneda" class="ew-search-field">
    <select
        id="x_moneda"
        name="x_moneda"
...
<div class="card ew-card ew-grid<?php if ($Page->isAddOrEdit()) { ?> ew-grid-add-edit<?php } ?> recarga">
<?php if (!$Page->isExport()) { ?>
...
<?php } ?>
<form name="frecargalist" id="frecargalist" class="form-inline ew-form ew-list-form" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<table id="tbl_recargalist" class="table ew-table"><!-- .ew-table -->
<thead>
...
<?php if ($Page->cliente->Visible) { // cliente ?>
        <th data-name="cliente" class="<?= $Page->cliente->headerCellClass() ?>"><div id="elh_recarga_cliente" class="recarga_cliente"><?= $Page->renderSort($Page->cliente) ?></div></th>
<?php } ?>
<?php if ($Page->fecha->Visible) { // fecha ?>
        <th data-name="fecha" class="<?= $Page->fecha->headerCellClass() ?>"><div id="elh_recarga_fecha" class="recarga_fecha"><?= $Page->renderSort($Page->fecha) ?></div></th>
<?php } ?>
<?php if ($Page->metodo_pago->Visible) { // metodo_pago ?>
        <th data-name="metodo_pago" class="<?= $Page->metodo_pago->headerCellClass() ?>"><div id="elh_recarga_metodo_pago" class="recarga_metodo_pago"><?= $Page->renderSort($Page->metodo_pago) ?></div></th>
<?php } ?>
<?php if ($Page->referencia->Visible) { // referencia ?>
        <th data-name="referencia" class="<?= $Page->referencia->headerCellClass() ?>"><div id="elh_recarga_referencia" class="recarga_referencia"><?= $Page->renderSort($Page->referencia) ?></div></th>
<?php } ?>
<?php if ($Page->monto_moneda->Visible) { // monto_moneda ?>
        <th data-name="monto_moneda" class="<?= $Page->monto_moneda->headerCellClass() ?>"><div id="elh_recarga_monto_moneda" class="recarga_monto_moneda"><?= $Page->renderSort($Page->monto_moneda) ?></div></th>
<?php } ?>
<?php if ($Page->moneda->Visible) { // moneda ?>
        <th data-name="moneda" class="<?= $Page->moneda->headerCellClass() ?>"><div id="elh_recarga_moneda" class="recarga_moneda"><?= $Page->renderSort($Page->moneda) ?></div></th>
<?php } ?>
<?php if ($Page->monto_bs->Visible) { // monto_bs ?>
        <th data-name="monto_bs" class="<?= $Page->monto_bs->headerCellClass() ?>"><div id="elh_recarga_monto_bs" class="recarga_monto_bs"><?= $Page->renderSort($Page->monto_bs) ?></div></th>
<?php } ?>
<?php if ($Page->tasa_usd->Visible) { // tasa_usd ?>
        <th data-name="tasa_usd" class="<?= $Page->tasa_usd->headerCellClass() ?>"><div id="elh_recarga_tasa_usd" class="recarga_tasa_usd"><?= $Page->renderSort($Page->tasa_usd) ?></div></th>
<?php } ?>
<?php if ($Page->monto_usd->Visible) { // monto_usd ?>
        <th data-name="monto_usd" class="<?= $Page->monto_usd->headerCellClass() ?>"><div id="elh_recarga_monto_usd" class="recarga_monto_usd"><?= $Page->renderSort($Page->monto_usd) ?></div></th>
<?php } ?>
<?php if ($Page->saldo->Visible) { // saldo ?>
        <th data-name="saldo" class="<?= $Page->saldo->headerCellClass() ?>"><div id="elh_recarga_saldo" class="recarga_saldo"><?= $Page->renderSort($Page->saldo) ?></div></th>
<?php } ?>
<?php if ($Page->_username->Visible) { // username ?>
        <th data-name="_username" class="<?= $Page->_username->headerCellClass() ?>"><div id="elh_recarga__username" class="recarga__username"><?= $Page->renderSort($Page->_username) ?></div></th>
<?php } ?>
<?php
...
while ($Page->RecordCount < $Page->StopRecord) {
    ...
    if ($Page->RecordCount >= $Page->StartRecord) {
        ...
        $Page->RowAttrs->merge(["data-rowindex" => $Page->RowCount, "id" => "r" . $Page->RowCount . "_recarga", "data-rowtype" => $Page->RowType]);
...
<span id="el<?= $Page->RowCount ?>_recarga_cliente">
...
<span id="el<?= $Page->RowCount ?>_recarga_fecha">
...
<span id="el<?= $Page->RowCount ?>_recarga_metodo_pago">
...
<span id="el<?= $Page->RowCount ?>_recarga_referencia">
...
<span id="el<?= $Page->RowCount ?>_recarga_monto_moneda">
...
<span id="el<?= $Page->RowCount ?>_recarga_moneda">
...
<span id="el<?= $Page->RowCount ?>_recarga_monto_bs">
...
<span id="el<?= $Page->RowCount ?>_recarga_tasa_usd">
...
<span id="el<?= $Page->RowCount ?>_recarga_monto_usd">
...
<span id="el<?= $Page->RowCount ?>_recarga_saldo">
...
<span id="el<?= $Page->RowCount ?>_recarga__username">
