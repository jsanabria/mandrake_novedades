		  ...
		  <?php 
		  	if($tipo_pago != "") {
				...
				<tr>
					...
					<td>
						<?= ($puede=='S' ? '<a class="btn btn-primary" id="agregar" name="agregar">Agregar</a>' : '') ?>
					</td>
				</tr>
				<?php
		  	}
			  		...
			  		if(trim($value) != "") {
			  			...
			  			if(count($arr2) > 2) {
					    	...
					    	$monto_usd = $monto_bs / $tasa_usd;
					  		...
					  		if($arr2[1] == "RD") $abono2 += floatval($arr2[4]);
			  			}
			  		}
			  	 } 
		  	}
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		if($("#tipo_pago").val() == "RC") {
			$("#disponible").val(($("#disponible").val().replace(",","")-$("#abono").val().replace(",","")).toFixed(2));
			// alert($("#disponible").val() + " | " + $("#abono").val());
		}

		if($("#tipo_pago").val() == "RD") {
			$("#disponible").val(($("#disponible").val().replace(",","")-$("#abono2").val().replace(",","")).toFixed(2));
			// alert($("#disponible").val() + " | " + $("#abono2").val());
		}
	});
	...
	$("#agregar").click(function() {
	  ...
	  var disponible = tipo_pago=="RC" ? $("#disponible").val() : (tipo_pago=="RD" ? $("#disponible").val() : 0);
	  var pagar = parseFloat($("#pagar").val());
	  var moneda = $("#moneda").val();
	  var monto = $("#monto").val();
	  var abono = $("#abono").val();
	  var abono2 = $("#abono2").val();
	  var continua = true;
	  ...
	  if(tipo_pago == "TC" || tipo_pago == "TD" || tipo_pago == "PM") { 
	  	if(!tieneSoloNumeros(referencia)) {
	  		alert("El NRO de referencia " + referencia + " solo debe tener valores numéricos.");
		  	return false;
	  	}
	  }
...
<script type="text/javascript">
	function tieneSoloNumeros(texto) {
	    //const regex = /[!@#$%^&*()_+=\[\]{};':\",<.>?~\\|]/; // Define la expresión regular con caracteres especiales
	    const regex = (/^[0-9]+$/); // Define la expresión regular con solo números
