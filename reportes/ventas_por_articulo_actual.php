...
$GLOBALS["titulo"] = "VENTAS POR ARTICULO DESDE " . $xFD[2] . "/" . $xFD[1] . "/" . $xFD[0] . " HASTA " . $xFH[2] . "/" . $xFH[1] . "/" . $xFH[0];
...
$sql = "SELECT 
					...
					d.ultimo_costo AS costo_unidad, SUM(b.cantidad_articulo*d.ultimo_costo) AS costo, b.precio_unidad, SUM(b.precio) AS precio, 
					((IFNULL(b.precio_unidad, 0)-IFNULL(d.ultimo_costo, 0))/IFNULL(b.precio_unidad,0))*100 AS utilidad 
				FROM 
					salidas AS a 
				...
				GROUP BY d.id, CONCAT(IFNULL(d.principio_activo, ' '), ' ', IFNULL(d.presentacion, ' '), ' ', IFNULL(d.nombre_comercial, ' ')), d.ultimo_costo, b.precio_unidad    
