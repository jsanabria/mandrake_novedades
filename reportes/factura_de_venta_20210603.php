...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(22, 5, "PRECIO", 1, 0, 'R');
		$this->Cell(10, 5, "DES %", 1, 0, 'R');
...
while($row = mysqli_fetch_array($rs))
{
	...
	if(strlen($row["articulo"]) < 36) 
		$pdf->Cell(57, 3, trim($row["articulo"]), 0, 0, 'L');
	else 
		$pdf->Cell(57, 3, substr(trim($row["articulo"]), 0, 36), 0, 0, 'L');
	...
	$pdf->Cell(22, 3, number_format($row["precio_ful"], 2, ",", "."), 0, 0, 'R');
	...
	if(strlen($row["articulo"]) >= 36) {
		$pdf->Ln();
		$pdf->Cell(30, 4);
		$pdf->MultiCell(57, 3, substr(trim($row["articulo"]), 36, strlen($row["articulo"])), 0, 'L');
	}
	else $pdf->Ln();
