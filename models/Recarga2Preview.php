 ...
 * Page class
 */
class Recarga2Preview extends Recarga2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Recarga2Preview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "abono2") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=abono2&" . GetForeignKeyUrl("fk_id", $this->abono->QueryStringValue) . "";
        }
        return $url;
    }
        ...
        if ($masterTblVar == "abono2") {
            ...
            if ($x !== false) {
                ...
                $val = $this->unquoteValue($val, "DB");
                 $this->abono->setQueryStringValue($val);
            }
        }
    }
    ...
    {
        // Example:
        $url = "ConfirmPage?page=REVERSO2&id=" . $this->id->CurrentValue;
        if($this->fecha->CurrentValue == date("Y-m-d") and !($this->metodo_pago->CurrentValue == "RD" or $this->metodo_pago->CurrentValue == "DV" or $this->metodo_pago->CurrentValue == "ND" or $this->metodo_pago->CurrentValue == "NC")) {
