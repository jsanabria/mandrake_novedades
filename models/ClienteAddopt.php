...
class ClienteAddopt extends Cliente
{
    ...
    public function run()
    {
        ...
        $this->puntos_refiere->setVisibility();
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->web->IsDetailKey) {
            $this->web->setFormValue(ConvertFromUtf8($val));
        }
        ...
        if (!$this->tarifa->IsDetailKey) {
            $this->tarifa->setFormValue(ConvertFromUtf8($val));
        }
        ...
        if (!$this->refiere->IsDetailKey) {
            $this->refiere->setFormValue(ConvertFromUtf8($val));
        }

        // Check field name 'puntos_refiere' first before field var 'x_puntos_refiere'
        $val = $CurrentForm->hasValue("puntos_refiere") ? $CurrentForm->getValue("puntos_refiere") : $CurrentForm->getValue("x_puntos_refiere");
        if (!$this->puntos_refiere->IsDetailKey) {
            $this->puntos_refiere->setFormValue(ConvertFromUtf8($val));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->web->CurrentValue = ConvertToUtf8($this->web->FormValue);
        $this->tarifa->CurrentValue = ConvertToUtf8($this->tarifa->FormValue);
        $this->refiere->CurrentValue = ConvertToUtf8($this->refiere->FormValue);
        $this->puntos_refiere->CurrentValue = ConvertToUtf8($this->puntos_refiere->FormValue);
    }
    ...
    protected function validateForm()
    {
        ...
        if ($this->puntos_refiere->Required) {
            if ($this->puntos_refiere->FormValue == "") {
                $this->puntos_refiere->addErrorMessage(str_replace("%s", $this->puntos_refiere->caption(), $this->puntos_refiere->RequiredErrorMessage));
            }
        }
