...
loadjs.ready("head", function () {
    ...
    // Form_CustomValidate
    fclienteadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fclienteadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fclienteadd.lists.web = <?= $Page->web->toClientList($Page) ?>;
    fclienteadd.lists.tarifa = <?= $Page->tarifa->toClientList($Page) ?>;
    fclienteadd.lists.refiere = <?= $Page->refiere->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fclienteadd" id="fclienteadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready("load", function () {
    // Startup script
    $("#x_ci_rif").change((function(){if(""==$("#x_ci_rif").val().trim())return!0;var i={ci_rif:$("#x_ci_rif").val(),tipo:"CLIENTE",accion:"I"};$.ajax({data:i,url:"RifBuscar",type:"get",beforeSend:function(){},success:function(i){return"1"!=$(i).find("#outtext").text()||(alert('RIF / CI "'+$("#x_ci_rif").val()+'" ya existe.'),$("#x_ci_rif").val(""),$("#x_ci_rif").focus(),!1)}})}));
});
</script>
