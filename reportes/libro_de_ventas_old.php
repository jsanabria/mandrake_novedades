...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(60, 5, "", "LTR", 0, 'L');
		$this->Cell(20, 5, "RIF", "LTR", 0, 'L');
		$this->Cell(24, 5, "TOTAL", "LTR", 0, 'R');
		$this->Cell(24, 5, "TOTAL", "LTR", 0, 'R');
		$this->Cell(52, 5, "DEBITO FISCAL", 1, 0, 'C');
		$this->Cell(20, 5, "TERCEROS", "LTR", 0, 'R');
		$this->Ln(5);
		...
		$this->Cell(60, 5, "NOMBRE O RAZON SOCIAL", "LBR", 0, 'L');
		...
		$this->Cell(20, 5, "IMPUESTO", "LBR", 0, 'R');
...
$sql = "SELECT 
		...
		WHERE 
			...
			a.fecha BETWEEN '$xfecha 00:00:00' AND '$yfecha 23:59:59' ORDER BY a.fecha;"; 
...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->Cell(60, 4, substr($row["cliente"], 0, 33), 0, 0, 'L');
	$pdf->Cell(20, 4, $row["ci_rif"], 0, 0, 'L');
	$pdf->Cell(24, 4, trim($row["estatus"])=="ANULADO" ? "" : ($row["total"]==0 ? "" : number_format(($row["documento"]=="NC" ? -1 : 1)*$row["total"], 2, ".", ",")), 0, 0, 'R');

	$sql = "SELECT SUM(IF(alicuota=0, precio, 0)) AS exenta FROM entradas_salidas 
	...
	$pdf->Cell(24, 4, trim($row["estatus"])=="ANULADO" ? "" : ($row2["exenta"]==0 ? "" : number_format(($row["documento"]=="NC" ? -1 : 1)*$row2["exenta"], 2, ".", ",")), 0, 0, 'R');

	$sql = "SELECT SUM(IF(alicuota>0, precio, 0)) AS gravable FROM entradas_salidas 
			WHERE id_documento = " . $row["id"] . " AND tipo_documento = '" . $row["tipo_documento"] . "'";
	$rs3 = mysqli_query($link, $sql) or die(mysqli_error());
	$row2 = mysqli_fetch_array($rs3);
	$pdf->Cell(24, 4, trim($row["estatus"])=="ANULADO" ? "" : ($row2["gravable"]==0 ? "" : number_format(($row["documento"]=="NC" ? -1 : 1)*$row2["gravable"], 2, ".", ",")), 0, 0, 'R');
