...
loadjs.ready("head", function () {
    ...
    farticuloadd = currentForm = new ew.Form("farticuloadd", "add");
    ...
    // Validate form
    farticuloadd.validate = function () {
    ...
    // Form_CustomValidate
    farticuloadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farticuloadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    farticuloadd.lists.fabricante = <?= $Page->fabricante->toClientList($Page) ?>;
    farticuloadd.lists.unidad_medida_defecto = <?= $Page->unidad_medida_defecto->toClientList($Page) ?>;
    farticuloadd.lists.cantidad_por_unidad_medida = <?= $Page->cantidad_por_unidad_medida->toClientList($Page) ?>;
    farticuloadd.lists.alicuota = <?= $Page->alicuota->toClientList($Page) ?>;
    farticuloadd.lists.articulo_inventario = <?= $Page->articulo_inventario->toClientList($Page) ?>;
    farticuloadd.lists.activo = <?= $Page->activo->toClientList($Page) ?>;
    farticuloadd.lists.sincroniza = <?= $Page->sincroniza->toClientList($Page) ?>;
    loadjs.done("farticuloadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="farticuloadd" id="farticuloadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
