...
class Salidas extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // ci_rif
        $this->ci_rif = new DbField('salidas', 'salidas', 'x_ci_rif', 'ci_rif', '`ci_rif`', '`ci_rif`', 200, 30, -1, false, '`ci_rif`', false, false, false, 'FORMATTED TEXT', 'RADIO');
        $this->ci_rif->Sortable = true; // Allow sort
        $this->ci_rif->Lookup = new Lookup('ci_rif', 'salidas', false, '', ["","","",""], [], [], [], [], [], [], '', '');
        $this->ci_rif->OptionCount = 2;
        ...
        // pago_premio
        $this->pago_premio = new DbField('salidas', 'salidas', 'x_pago_premio', 'pago_premio', '`pago_premio`', '`pago_premio`', 202, 1, -1, false, '`pago_premio`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        $this->pago_premio->Sortable = true; // Allow sort
        $this->pago_premio->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->pago_premio->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->pago_premio->Lookup = new Lookup('pago_premio', 'salidas', false, '', ["","","",""], [], [], [], [], [], [], '', '');
        $this->pago_premio->OptionCount = 2;
        $this->pago_premio->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->pago_premio->Param, "CustomMsg");
        $this->Fields['pago_premio'] = &$this->pago_premio;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->pago_premio->DbValue = $row['pago_premio'];
    }
...
SORTHTML;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->pago_premio);
                } else {
                    $doc->exportCaption($this->tipo_documento);
                    $doc->exportCaption($this->nro_documento);
                    $doc->exportCaption($this->nro_control);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->pago_premio);
                    } else {
                        $doc->exportField($this->tipo_documento);
                        $doc->exportField($this->nro_documento);
                        $doc->exportField($this->nro_control);
    ...
    public function rowInserting($rsold, &$rsnew) {
    	...
    	case "TDCNET":
    		...
    		// Tomo el porcentaje de descuento cashea
    		if(trim($rsnew["ci_rif"]) == "S") {
    			$sql = "SELECT valor1 AS descuento FROM parametro WHERE codigo = '056';";
    			if($row = ExecuteRow($sql)) 
    				$rsnew["descuento"] = floatval($row["descuento"]);
    		}
    		break;
    	case "TDCFCV":
    	...
    	elseif($rsold["tipo_documento"] == "TDCFCV") {
    		...
    		if($rsnew["igtf"] == "S") {
    			if(floatval($rsnew["monto_base_igtf"]) > 0) {
    				...
    				$this->CancelMessage = "El monto para aplicar IGTF debe ser mayor a 0.";
    				return FALSE;
    			}
    		}
    		else {
    			...
    			if($rsnew["estatus"] == "ANULADO") {
    				...
    				foreach ($rowsPagos as $keyPago => $rowPago) {
                        ...
                        $sql = "SELECT
                                    ...
                                    nota, username 
                                FROM recarga WHERE cobro_cliente_reverso = $idPago";
                        $rows = ExecuteRows($sql);
                        foreach ($rows as $key => $row) {
                        	$sql = "SELECT IFNULL(MAX(nro_recibo), 0)+1 AS nro FROM abono WHERE 1;";
                        	$nro_recibo = 0; // ExecuteScalar($sql);
    				...
    				$cliente = $rsold["cliente"];
    				$sql = "SELECT 
    							tipo, referencia, puntos, nota
    						FROM 
    							puntos 
    						WHERE 
    							cliente = $cliente AND nro_documento = '$nro_documento';";
    				$rows = ExecuteRows($sql);	
    				foreach ($rows as $key => $value) {
    					$ref = $value["referencia"];
    					$puntos = (-1) * intval($value["puntos"]);
    					$nota = "SE ANULA N.E. " . $value["nota"];
    					$sql = "INSERT INTO puntos
    								(id, cliente, fecha, tipo, nro_documento, referencia, puntos, saldo, nota, username)
    							VALUES
    								(NULL, $cliente, '" . date("Y-m-d") . "', 'NE', '$nro_documento', '$ref', $puntos, 0, '$nota', '" . CurrentUserName() . "')";
    					Execute($sql);
    				...
    				/*** Se evalua si se reversan puntos por referido en la nota por la anulación de la misma ***/
    				$nro_documento = $rsold["nro_documento"];
    				$cliente = $rsold["cliente"];
    				$sql = "SELECT IFNULL(refiere, 0) AS refiere FROM cliente WHERE id = $cliente;";
    				$row = ExecuteRow($sql);
    				$refiere = $row["refiere"];
    				$sql = "SELECT 
    							tipo, referencia, puntos, nota
    						FROM 
    							puntos 
    						WHERE 
    							cliente = $refiere AND nro_documento = '$nro_documento';"; 
    				$rows = ExecuteRows($sql);	
    				foreach ($rows as $key => $value) {
    					$ref = $value["referencia"];
    					$puntos = (-1) * intval($value["puntos"]);
    					$nota = "SE ANULA N.E. " . $value["nota"] . " Y DEVUELVE PUNTOS POR REFERIDO";
    					$sql = "INSERT INTO puntos
    								(id, cliente, fecha, tipo, nro_documento, referencia, puntos, saldo, nota, username)
    							VALUES
    								(NULL, $refiere, '" . date("Y-m-d") . "', 'NE', '$nro_documento', '$ref', $puntos, 0, '$nota', '" . CurrentUserName() . "')";
    					Execute($sql);
    					$sql = "SELECT IFNULL(SUM(puntos), 0) AS saldo FROM puntos
    							WHERE cliente = $refiere;";
    					$saldo = ExecuteScalar($sql);
    					$sql = "SELECT LAST_INSERT_ID() AS id;";
    					$id = ExecuteScalar($sql);
    					$sql = "UPDATE puntos SET saldo = $saldo WHERE id = $id;";
