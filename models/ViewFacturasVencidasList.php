...
class ViewFacturasVencidasList extends ViewFacturasVencidas
{
    ...
    // Table name
    public $TableName = 'view_facturas_vencidas';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["view_facturas_vencidas"]) || get_class($GLOBALS["view_facturas_vencidas"]) == PROJECT_NAMESPACE . "view_facturas_vencidas") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fview_facturas_vencidaslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("view_facturas_vencidas"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fview_facturas_vencidaslistsrch", $filters);
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->cliente, $arKeywords, $type);
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`codcli` ASC,`dias_vencidos` DESC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->codcli->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($this->dias_vencidos->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->codcli->setSort("ASC");
                    $this->dias_vencidos->setSort("DESC");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_facturas_vencidaslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_facturas_vencidaslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fview_facturas_vencidaslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // codcli
        if (!$this->isAddOrEdit() && $this->codcli->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // dias_vencidos
        if (!$this->isAddOrEdit() && $this->dias_vencidos->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->dias_vencidos->CssClass = "font-weight-bold font-italic";
            ...
            if ($curVal != "") {
                ...
                $this->codcli->AdvancedSearch->ViewValue = $this->codcli->Lookup !== null && is_array($this->codcli->Lookup->Options) ? $curVal : null;
            }
            if ($this->codcli->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->codcli->EditValue = array_values($this->codcli->Lookup->Options);
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`id`" . SearchString("=", $this->codcli->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
            ...
            $this->fecha_entrega->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_entrega->AdvancedSearch->SearchValue, 7), 7));
            ...
            $this->dias_credito->EditValue = HtmlEncode($this->dias_credito->AdvancedSearch->SearchValue);
            $this->dias_credito->PlaceHolder = RemoveHtml($this->dias_credito->caption());

            // dias_vencidos
            $this->dias_vencidos->EditAttrs["class"] = "form-control";
            $this->dias_vencidos->EditCustomAttributes = "";
            $this->dias_vencidos->EditValue = HtmlEncode($this->dias_vencidos->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fview_facturas_vencidaslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fview_facturas_vencidaslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fview_facturas_vencidaslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_view_facturas_vencidas" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_view_facturas_vencidas\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fview_facturas_vencidaslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fview_facturas_vencidaslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
