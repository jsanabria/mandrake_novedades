...
class EntradasView extends Entradas
{
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            ...
            $returnUrl = "EntradasList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'entradas')) {
    ...
    public function renderRow()
    {
        ...
        // Convert decimal values if posted back
        if ($this->ret_iva->FormValue == $this->ret_iva->CurrentValue && is_numeric(ConvertToFloatString($this->ret_iva->CurrentValue))) {
            $this->ret_iva->CurrentValue = ConvertToFloatString($this->ret_iva->CurrentValue);
        }

        // Convert decimal values if posted back
        if ($this->ret_islr->FormValue == $this->ret_islr->CurrentValue && is_numeric(ConvertToFloatString($this->ret_islr->CurrentValue))) {
            $this->ret_islr->CurrentValue = ConvertToFloatString($this->ret_islr->CurrentValue);
        }

        // Convert decimal values if posted back
        if ($this->ret_municipal->FormValue == $this->ret_municipal->CurrentValue && is_numeric(ConvertToFloatString($this->ret_municipal->CurrentValue))) {
            $this->ret_municipal->CurrentValue = ConvertToFloatString($this->ret_municipal->CurrentValue);
        }
    ...
    public function pageRender() {
    	...
    	case "TDCFCC":
    		...
    		$this->tipo_iva->Visible = TRUE;
    		$this->tipo_islr->Visible = TRUE;
    		$this->sustraendo->Visible = TRUE;
    		$this->tipo_municipal->Visible = TRUE;
    		$this->descuento->Visible = TRUE;
    		$this->monto_total->Visible = TRUE;
    		$this->alicuota_iva->Visible = TRUE;
    		$this->iva->Visible = TRUE;
    ...
    public function pageDataRendering(&$header) {
    	...
    	if($this->tipo_documento->CurrentValue == "TDCNRP") {
    		$sql = "SELECT id
    				FROM recarga 
    				WHERE nota_recepcion = " . $this->id->CurrentValue . ";";
    		$rows = ExecuteRows($sql);
    		foreach ($rows as $key => $value) {
    			$url = "../reportes/rptRecibo.php?id=" . $value["id"] . "";
    			$header .= '<a class="btn btn-primary" target="_blank" href="' . $url . '"><span class="fas fa-print"></span> Recibo de Abono # ' . $value["id"] . '</a>&nbsp&nbsp';
    		}
    	}
    	...
    	case "TDCPDC":
    		...
    		$header .= '<a class="btn btn-primary" href="' . $url . '"><span class="fas fa-edit"></span> Editar Documento "' . $tipo_name . '"</a>';
    		break;
    	case "TDCNRP":
    ...
    public function pageDataRendered(&$footer)
    {
        ...
        $footer = "<strong>Total Unidades: $unidades</strong>";	
    }
