...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(45, 6, "CLIENTE", 1, 0, 'L');
		$this->Cell(20, 6, "VENDEDOR", 1, 0, 'L');
	...
	function EndReport($xTotalCosto, $xTotalPrecio, $xTotalUtilidad, $xitems, $xxfecha, $xyfecha)
	{
		...
		$sql = "SELECT 
					d.nombre AS vendedor, IFNULL(SUM(b.costo), 0) AS costo, SUM(b.precio) AS precio  
				FROM 
					salidas AS a 
		...
		{
			$this->Ln();
			$this->Cell(130, 4, $row["vendedor"], 0, 0, 'R');
...
$sql = "SELECT 
			...
			a.monto_total, IFNULL(SUM(b.costo), 0) AS costo, SUM(b.precio) AS precio, d.nombre AS vendedor  
		FROM 
			salidas AS a 
			JOIN entradas_salidas AS b ON b.tipo_documento = a.tipo_documento AND b.id_documento = a.id 
			LEFT OUTER JOIN cliente AS c ON c.id = a.cliente 
		...
		WHERE 
			...
			AND a.estatus = 'PROCESADO' $xasesor $xcliente  $xuser 
		GROUP BY a.username, a.fecha, c.nombre, a.nro_documento, a.moneda, a.monto_total, d.nombre 
...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->Cell(45, 4, utf8_decode(substr($row["cliente"], 0, 30)), 0, 0, 'L');
	$pdf->Cell(20, 4, utf8_decode(substr($row["vendedor"], 0, 10)), 0, 0, 'L');
