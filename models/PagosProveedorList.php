...
class PagosProveedorList extends PagosProveedor
{
    ...
    // Table name
    public $TableName = 'pagos_proveedor';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["pagos_proveedor"]) || get_class($GLOBALS["pagos_proveedor"]) == PROJECT_NAMESPACE . "pagos_proveedor") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fpagos_proveedorlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("pagos_proveedor"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpagos_proveedorlistsrch", $filters);
    ...
    protected function setupListOptions()
    {
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'pagos_proveedor_factura') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'pagos_proveedor_factura')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("PagosProveedorFacturaList?" . Config("TABLE_SHOW_MASTER") . "=pagos_proveedor&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpagos_proveedorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpagos_proveedorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fpagos_proveedorlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`pagos_proveedor`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";

        // Column "detail_pagos_proveedor_factura"
        if ($this->DetailPages && $this->DetailPages["pagos_proveedor_factura"] && $this->DetailPages["pagos_proveedor_factura"]->Visible && $Security->allowList(CurrentProjectID() . 'pagos_proveedor_factura')) {
            $link = "";
            $option = $this->ListOptions["detail_pagos_proveedor_factura"];
            $url = "PagosProveedorFacturaPreview?t=pagos_proveedor&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"pagos_proveedor_factura\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'pagos_proveedor')) {
                $label = $Language->TablePhrase("pagos_proveedor_factura", "TblCaption");
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"pagos_proveedor_factura\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("PagosProveedorFacturaList?" . Config("TABLE_SHOW_MASTER") . "=pagos_proveedor&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . $Language->TablePhrase("pagos_proveedor_factura", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</a>";
            }
    ...
    protected function loadSearchValues()
    {
        ...
        // banco
        if (!$this->isAddOrEdit() && $this->banco->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // monto_dado
        if (!$this->isAddOrEdit() && $this->monto_dado->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // monto
        if (!$this->isAddOrEdit() && $this->monto->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // cont_lotes
        if (!$this->isAddOrEdit() && $this->cont_lotes->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    public function renderRow()
    {
        ...
        if ($this->monto_dado->FormValue == $this->monto_dado->CurrentValue && is_numeric(ConvertToFloatString($this->monto_dado->CurrentValue))) {
            $this->monto_dado->CurrentValue = ConvertToFloatString($this->monto_dado->CurrentValue);
        }
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->proveedor->ViewValue === null) { // Lookup from database
                    ...
                    $lookupFilter = function() {
                        return (CurrentPageID() == "add") ? "`id` IN (SELECT proveedor FROM view_x_pagar WHERE IFNULL(monto_pagar, 0) <> IFNULL(monto_pagado, 0) AND fecha > '2021-07-31')" : "";
                    };
            ...
            $this->monto_dado->ViewValue = FormatNumber($this->monto_dado->ViewValue, 2, -1, -1, -1);
            $this->monto_dado->ViewCustomAttributes = "";
            ...
            $this->cont_lotes->ViewValue = FormatNumber($this->cont_lotes->ViewValue, 0, -2, -2, -2);
            $this->cont_lotes->ViewCustomAttributes = "";
            ...
            // monto_dado
            $this->monto_dado->LinkCustomAttributes = "";
            ...
            if ($curVal != "") {
                ...
                $this->proveedor->AdvancedSearch->ViewValue = $this->proveedor->Lookup !== null && is_array($this->proveedor->Lookup->Options) ? $curVal : null;
            }
            if ($this->proveedor->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`id`" . SearchString("=", $this->proveedor->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $lookupFilter = function() {
                    return (CurrentPageID() == "add") ? "`id` IN (SELECT proveedor FROM view_x_pagar WHERE IFNULL(monto_pagar, 0) <> IFNULL(monto_pagado, 0) AND fecha > '2021-07-31')" : "";
                };
            ...
            // monto_dado
            $this->monto_dado->EditAttrs["class"] = "form-control";
            $this->monto_dado->EditCustomAttributes = "";
            $this->monto_dado->EditValue = HtmlEncode($this->monto_dado->AdvancedSearch->SearchValue);
            $this->monto_dado->PlaceHolder = RemoveHtml($this->monto_dado->caption());

            // monto
            $this->monto->EditAttrs["class"] = "form-control";
            $this->monto->EditCustomAttributes = "";
            $this->monto->EditValue = HtmlEncode($this->monto->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fpagos_proveedorlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fpagos_proveedorlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fpagos_proveedorlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_pagos_proveedor" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_pagos_proveedor\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fpagos_proveedorlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpagos_proveedorlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
