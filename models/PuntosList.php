 ...
 * Page class
 */
class PuntosList extends Puntos
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'puntos';

    // Page object name
    public $PageObjName = "PuntosList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fpuntoslist";
    ...
    public function __construct()
    {
        ...
        // Table object (puntos)
        if (!isset($GLOBALS["puntos"]) || get_class($GLOBALS["puntos"]) == PROJECT_NAMESPACE . "puntos") {
            $GLOBALS["puntos"] = &$this;
        }
        ...
        $this->AddUrl = "PuntosAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "PuntosDelete";
        $this->MultiUpdateUrl = "PuntosUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'puntos');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fpuntoslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("puntos"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->referencia);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->puntos->AdvancedSearch->toJson(), ","); // Field puntos
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpuntoslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field puntos
        $this->puntos->AdvancedSearch->SearchValue = @$filter["x_puntos"];
        $this->puntos->AdvancedSearch->SearchOperator = @$filter["z_puntos"];
        $this->puntos->AdvancedSearch->SearchCondition = @$filter["v_puntos"];
        $this->puntos->AdvancedSearch->SearchValue2 = @$filter["y_puntos"];
        $this->puntos->AdvancedSearch->SearchOperator2 = @$filter["w_puntos"];
        $this->puntos->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->puntos, $default, false); // puntos
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->puntos->AdvancedSearch->save(); // puntos
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->puntos->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->puntos->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        // Restore advanced search values
                ...
                $this->puntos->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->puntos); // puntos
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpuntoslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpuntoslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fpuntoslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // tipo
        if (!$this->isAddOrEdit() && $this->tipo->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // puntos
        if (!$this->isAddOrEdit() && $this->puntos->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->puntos->AdvancedSearch->SearchValue != "" || $this->puntos->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->referencia->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->referencia->EditValue = $this->referencia->lookupCacheOption($curVal);
                if ($this->referencia->EditValue === null) { // Lookup from database
                    $filterWrk = "`codigo_ims`" . SearchString("=", $curVal, DATATYPE_STRING, "");
                    $sqlWrk = $this->referencia->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->referencia->Lookup->renderViewRow($rswrk[0]);
                        $this->referencia->EditValue = $this->referencia->displayValue($arwrk);
                    } else {
                        $this->referencia->EditValue = HtmlEncode($this->referencia->AdvancedSearch->SearchValue);
                    }
                }
            } else {
                $this->referencia->EditValue = null;
            }
            ...
            $this->puntos->EditValue = HtmlEncode($this->puntos->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fpuntoslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fpuntoslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fpuntoslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_puntos" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_puntos\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fpuntoslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpuntoslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    {
        // Example:
        $opt = &$this->ListOptions->Add("estado_cuenta");
        $opt->Header = "EDO CTA";
        $opt->OnLeft = true; // Link on left
        $opt->MoveTo(3); // Move to first column

        /*
        $opt = &$this->ListOptions->Add("recibo_premio");
        $opt->Header = "RECIBO";
        $opt->OnLeft = true; // Link on left
    ...
    {
        // Example:
       	$this->ListOptions["estado_cuenta"]->Body = '<center><a class="btn btn-primary" target="_blank" href="reportes/rptEstadoCuenta.php?cliente=' . $this->cliente->CurrentValue . '"><span class="fas fa-print"></span></a></center>';

       	/*
       	if($this->tipo->CurrentValue == "PP")
       		$this->ListOptions["recibo_premio"]->Body = '<center><a class="btn btn-primary" target="_blank" href="reportes/rptReciboPremio.php?id=' . $this->id->CurrentValue . '"><span class="fas fa-print"></span></a></center>';
       	else
       		$this->ListOptions["recibo_premio"]->Body = '';
       	*/
    }
