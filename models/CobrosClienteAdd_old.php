        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->tipo_pago->ViewValue === null) { // Lookup from database
                    ...
                    $lookupFilter = function() {
                        return (isset($_REQUEST["dsc"]) ? ($_REQUEST["dsc"] >= 25 ? "`codigo` = '009' AND valor1 IN ('EF','RC','RD')" : "`codigo` = '009' AND valor1 NOT IN ('PC','PF','DV','NC','ND')") : "`codigo` = '009' AND valor1 NOT IN ('PC','PF','DV','NC','ND')");
                    };
                    $lookupFilter = $lookupFilter->bindTo($this);
    ...
    public function pageLoad()
    {
        ...
        if($id > 0) { 
    		...
    		if($saldo > 0.00) {
    			...
    			$tipo_name = '<span class="text-danger"><strong>Saldo: </strong>$ ' . number_format($saldo, 2, ".", ",") . '<input type="hidden" id=" value="S"></span>';
    		}
