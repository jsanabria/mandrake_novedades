...
class PDF extends FPDF
{
	...
	function EndReport($id_invoice)
	{
		...
		$moneda = ($moneda == "USD" ? "USD" : $moneda); // Junior USD
	...
	function BodyReport() 
	{
		...
		if($row = mysqli_fetch_array($rs)) $saldo_actual = "\n(Saldo actual abonos: *** USD " . number_format($row["saldo"], 2, ".", ",") . " ***)"; // Junior USD
		...
		$this->Cell(60, 4);
		$this->Cell(70, 4, $saldo_actual, 0, 0, 'C');
		//$pdf->Cell(10, 4);
		$this->Ln();
		...
		while($row = mysqli_fetch_array($rs))
		{
			...
			$this->Cell(20, 4, ($row["moneda"] == "USD" ? "USD" : $row["moneda"]) . " " . $row["monto_moneda"], 0, 0, 'R');  // Junior USD
