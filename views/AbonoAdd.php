...
loadjs.ready("head", function () {
    ...
    fabonoadd = currentForm = new ew.Form("fabonoadd", "add");
    ...
    // Form_CustomValidate
    fabonoadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fabonoadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fabonoadd.lists.cliente = <?= $Page->cliente->toClientList($Page) ?>;
    fabonoadd.lists.metodo_pago = <?= $Page->metodo_pago->toClientList($Page) ?>;
    loadjs.done("fabonoadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fabonoadd" id="fabonoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->cliente->Visible) { // cliente ?>
    <div id="r_cliente" class="form-group row">
        <label id="elh_abono_cliente" class="<?= $Page->LeftColumnClass ?>"><?= $Page->cliente->caption() ?><?= $Page->cliente->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
loadjs.ready(["fabonoadd"], function() {
    fabonoadd.createAutoSuggest(Object.assign({"id":"x_cliente","forceSelect":true}, ew.vars.tables.abono.fields.cliente.autoSuggestOptions));
});
</script>
...
loadjs.ready("load", function () {
    // Startup script
    $("#x_pago").prop("readonly",!0),$("#r_pivote").hide(),$("#btn-action").prop("disabled",!0);var pivote2="<input type='hidden' id='pagos' name='pagos' value=''>";$("#r_pivote2").html(pivote2),$("#x_metodo_pago").change((function(){$("#r_pivote").show();var o=$("#x_cliente").val(),e=$("#x_metodo_pago").val(),t=$("#pagos").val();if($("#r_pivote").html(""),""==o)return alert("Seleccione un cliente"),location.reload(),!0;$.ajax({url:"include/Cliente_Tipo_Pago_Abonos.php",type:"GET",data:{cliente:o,tipo_pago:e,pagos:t},beforeSend:function(){$("#r_pivote").html("Por Favor Espere. . .")}}).done((function(o){var e;e=o,$("#r_pivote").html(e)})).fail((function(o){alert("error"+o)})).always((function(o){}))})),$("#x_cliente").change((function(){$("#btn-action").prop("disabled",!0)}));
});
</script>
