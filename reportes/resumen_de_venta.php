...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(17, 6, "FECHA", 1, 0, 'L');
		$this->Cell(15, 6, "DOC.", 1, 0, 'L');
		$this->Cell(58, 6, "CLIENTE", 1, 0, 'L');
	...
	function EndReport($Total, $granTotal, $items, $xfecha, $yfecha, $articulos)
	{
		//$this->AddPage();
		//require("../connect.php");
		$this->SetFont('Arial','B',8);
		$this->Ln();
		$this->Cell(200, 5, "TOTAL ARTICULOS: "  . number_format($articulos, 0, ".", ","), 0, 0, 'R');
		$this->Ln();
		$this->Cell(200, 5, "TOTAL PEDIDOS: "  . $items, 0, 0, 'R');
		$this->Ln();
...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->Cell(17, 5, $row["fecha"], 0, 0, 'L');
	$pdf->Cell(15, 5, $row["nro_documento"], 0, 0, 'L');
	$pdf->Cell(58, 5, substr($row["cliente"], 0, 30), 0, 0, 'L');

	$sql2 = "SELECT SUM(cantidad_articulo) AS cantidad FROM entradas_salidas WHERE tipo_documento = 'TDCPDV' AND id_documento = " . $row["id"] . ";";
	$rs2 = mysqli_query($link, $sql2) or die(mysqli_error());
	$row2 = mysqli_fetch_array($rs2);

	$pdf->Cell(10, 5, number_format($row2["cantidad"], 0, ".", ","), 0, 0, 'R');
	$articulos += intval($row2["cantidad"]);
...
$pdf->EndReport($Total, $granTotal, $items, $xfecha, $yfecha, $articulos);
