...
class TarifaAnteriorList extends TarifaAnterior
{
    ...
    // Table name
    public $TableName = 'tarifa_anterior';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["tarifa_anterior"]) || get_class($GLOBALS["tarifa_anterior"]) == PROJECT_NAMESPACE . "tarifa_anterior") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option ftarifa_anteriorlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("tarifa_anterior"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ftarifa_anteriorlistsrch", $filters);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ftarifa_anteriorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ftarifa_anteriorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.ftarifa_anteriorlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // articulo
        if (!$this->isAddOrEdit() && $this->articulo->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // precio_anterior
        if (!$this->isAddOrEdit() && $this->precio_anterior->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // precio_nuevo
        if (!$this->isAddOrEdit() && $this->precio_nuevo->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    protected function newRow()
    {
        ...
        $row['fabricante'] = null;
        $row['articulo'] = null;
        $row['precio_anterior'] = null;
        $row['precio_nuevo'] = null;
        $row['activo'] = null;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->precio_anterior->FormValue == $this->precio_anterior->CurrentValue && is_numeric(ConvertToFloatString($this->precio_anterior->CurrentValue))) {
            $this->precio_anterior->CurrentValue = ConvertToFloatString($this->precio_anterior->CurrentValue);
        }

        // Convert decimal values if posted back
        if ($this->precio_nuevo->FormValue == $this->precio_nuevo->CurrentValue && is_numeric(ConvertToFloatString($this->precio_nuevo->CurrentValue))) {
            $this->precio_nuevo->CurrentValue = ConvertToFloatString($this->precio_nuevo->CurrentValue);
        }
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->precio_anterior->ViewValue = FormatNumber($this->precio_anterior->ViewValue, 2, -1, -1, -1);
            $this->precio_anterior->ViewCustomAttributes = "";

            // precio_nuevo
            $this->precio_nuevo->ViewValue = $this->precio_nuevo->CurrentValue;
            $this->precio_nuevo->ViewValue = FormatNumber($this->precio_nuevo->ViewValue, 2, -1, -1, -1);
            $this->precio_nuevo->ViewCustomAttributes = "";
            ...
            // precio_anterior
            $this->precio_anterior->LinkCustomAttributes = "";
            $this->precio_anterior->HrefValue = "";
            $this->precio_anterior->TooltipValue = "";

            // precio_nuevo
            $this->precio_nuevo->LinkCustomAttributes = "";
            ...
            $this->articulo->EditValue = HtmlEncode($this->articulo->AdvancedSearch->SearchValue);
            $this->articulo->PlaceHolder = RemoveHtml($this->articulo->caption());

            // precio_anterior
            $this->precio_anterior->EditAttrs["class"] = "form-control";
            $this->precio_anterior->EditCustomAttributes = "";
            $this->precio_anterior->EditValue = HtmlEncode($this->precio_anterior->AdvancedSearch->SearchValue);
            $this->precio_anterior->PlaceHolder = RemoveHtml($this->precio_anterior->caption());

            // precio_nuevo
            $this->precio_nuevo->EditAttrs["class"] = "form-control";
            $this->precio_nuevo->EditCustomAttributes = "";
            $this->precio_nuevo->EditValue = HtmlEncode($this->precio_nuevo->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.ftarifa_anteriorlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.ftarifa_anteriorlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.ftarifa_anteriorlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_tarifa_anterior" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_tarifa_anterior\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.ftarifa_anteriorlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ftarifa_anteriorlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
