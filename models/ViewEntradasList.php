...
class ViewEntradasList extends ViewEntradas
{
    ...
    // Table name
    public $TableName = 'view_entradas';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["view_entradas"]) || get_class($GLOBALS["view_entradas"]) == PROJECT_NAMESPACE . "view_entradas") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fview_entradaslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("view_entradas"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fview_entradaslistsrch", $filters);
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->nombre_documento, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->nombre_proveedor, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->nro_documento, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->fecha, $arKeywords, $type);
    ...
    protected function setupListOptions()
    {
        ...
        $item->Visible = $Security->allowList(CurrentProjectID() . 'view_entradas_salidas') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'view_entradas_salidas')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("ViewEntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=view_entradas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->CurrentValue) . "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_entradaslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_entradaslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fview_entradaslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages && $this->DetailPages["view_entradas_salidas"] && $this->DetailPages["view_entradas_salidas"]->Visible && $Security->allowList(CurrentProjectID() . 'view_entradas_salidas')) {
            $link = "";
            $option = $this->ListOptions["detail_view_entradas_salidas"];
            $url = "ViewEntradasSalidasPreview?t=view_entradas&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"view_entradas_salidas\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'view_entradas')) {
                $label = $Language->TablePhrase("view_entradas_salidas", "TblCaption");
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"view_entradas_salidas\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("ViewEntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=view_entradas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->CurrentValue) . "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . $Language->TablePhrase("view_entradas_salidas", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</a>";
            }
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_entradas')) {
                ...
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_entradas_salidas");
    ...
    protected function loadSearchValues()
    {
        ...
        // nombre_documento
        if (!$this->isAddOrEdit() && $this->nombre_documento->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // nombre_proveedor
        if (!$this->isAddOrEdit() && $this->nombre_proveedor->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        if (!$this->isAddOrEdit() && $this->consignacion_reportada->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    protected function newRow()
    {
        ...
        $row['nombre_documento'] = null;
        $row['proveedor'] = null;
        $row['nombre_proveedor'] = null;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fview_entradaslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fview_entradaslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fview_entradaslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_view_entradas" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_view_entradas\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fview_entradaslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fview_entradaslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
