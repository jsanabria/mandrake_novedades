 ...
 * Page class
 */
class Abono2List extends Abono2
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'abono2';

    // Page object name
    public $PageObjName = "Abono2List";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fabono2list";
    ...
    public function __construct()
    {
        ...
        // Table object (abono2)
        if (!isset($GLOBALS["abono2"]) || get_class($GLOBALS["abono2"]) == PROJECT_NAMESPACE . "abono2") {
            $GLOBALS["abono2"] = &$this;
        }
        ...
        $this->AddUrl = "Abono2Add?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "Abono2Delete";
        $this->MultiUpdateUrl = "Abono2Update";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'abono2');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fabono2listsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("abono2"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fabono2listsrch", $filters);
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_recarga2"
        $item = &$this->ListOptions->add("detail_recarga2");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'recarga2') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_recarga2"
        $opt = $this->ListOptions["detail_recarga2"];
        if ($Security->allowList(CurrentProjectID() . 'recarga2')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("recarga2", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("Recarga2List?" . Config("TABLE_SHOW_MASTER") . "=abono2&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("Recarga2Grid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'abono2')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=recarga2");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "recarga2";
            }
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_recarga2");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=recarga2");
                $detailPage = Container("Recarga2Grid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'abono2') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "recarga2";
                }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fabono2listsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fabono2listsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fabono2list},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`abono`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";

        // Column "detail_recarga2"
        if ($this->DetailPages && $this->DetailPages["recarga2"] && $this->DetailPages["recarga2"]->Visible && $Security->allowList(CurrentProjectID() . 'recarga2')) {
            $link = "";
            $option = $this->ListOptions["detail_recarga2"];
            $url = "Recarga2Preview?t=abono2&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"recarga2\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'abono2')) {
                $label = $Language->TablePhrase("recarga2", "TblCaption");
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"recarga2\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("Recarga2List?" . Config("TABLE_SHOW_MASTER") . "=abono2&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . $Language->TablePhrase("recarga2", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</a>";
            }
            $detailPageObj = Container("Recarga2Grid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'abono2')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=recarga2");
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->metodo_pago->ViewValue === null) { // Lookup from database
                    ...
                    $lookupFilter = function() {
                        return CurrentPageID() == "add" ? ("codigo = '009' AND valor1 NOT IN " . (CurrentUserLevel() == -1 || CurrentUserLevel() == 6 ? "('RC', 'RD', 'DV', 'PM', 'PC')" : "('RC', 'RD', 'DV', 'PM', 'PC', 'NC', 'ND', 'PF')")) : "codigo = '009'";
                    };
            ...
            $this->nro_recibo->EditValue = HtmlEncode($this->nro_recibo->AdvancedSearch->SearchValue);
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->nro_recibo->AdvancedSearch->SearchValue)) {
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fabono2list, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fabono2list, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fabono2list, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_abono2" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_abono2\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fabono2list, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fabono2listsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageDataRendering(&$header)
    {
    	...
    	if (CurrentUserLevel() == -1 || CurrentUserLevel() == 6) {
        	// Agregamos un botón con estilo simple de Bootstrap (clase btn-default o btn-primary)
        	$header .= '<a href="reportes/Reporte_Notas_Credito.php" target="_blank" class="btn btn-primary" style="margin-bottom: 10px;">
                        <i class="fa fa-file-pdf-o"></i> Reporte de Notas de Crédito
                    </a>';
        }
    }
    ...
    public function listOptionsRendered()
    {
        ...
        if($this->id->CurrentValue != "") {
        	$sql = "SELECT saldo FROM recarga2 WHERE abono = " . $this->id->CurrentValue . " ORDER BY id DESC LIMIT 0, 1;";
        ...
        if($this->metodo_pago->CurrentValue == "IMPRIMIR")
        	$this->ListOptions["print"]->Body = '<a class="btn btn-primary" target="_blank" href="reportes/rptRecibo2.php?id=' . $this->id->CurrentValue . '"><span class="fas fa-print"></span></a>';
    }
