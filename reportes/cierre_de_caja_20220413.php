...
$pdf->Cell(200, 6, "INGRESOS POR TIPO DE PAGO", 0, 0, 'C');
$pdf->SetFont('Arial','BI',8);
$pdf->Ln();
$pdf->Cell(10, 6);
$pdf->Cell(30, 6, "TIPO DE PAGO", 1, 0, 'L');
$pdf->Cell(30, 6, "DOCUMENTO", 1, 0, 'L');
$pdf->Cell(20, 6, "NRO DOC", 1, 0, 'C');
$pdf->Cell(50, 6, "CLIENTE", 1, 0, 'L');
$pdf->Cell(30, 6, "MONTO BS", 1, 0, 'R');
$pdf->Cell(30, 6, "MONTO USD", 1, 0, 'R');
$pdf->Ln(6);

$sql = "SELECT 
			aa.tipo, 
			bb.valor2 AS metodo_pago, 
			aa.doc, 
			aa.cliente, 
			IF(aa.metodo_pago='PF' OR aa.metodo_pago='PC', (-1), 1)*aa.monto_bs AS monto_bs, 
			IF(aa.metodo_pago='PF' OR aa.metodo_pago='PC', (-1), 1)*aa.monto_usd AS monto_usd, 
			aa.metodo_pago AS tipo_pago 
		FROM (
		SELECT 
			...
			d.nombre AS cliente,  
			b.monto_bs AS monto_bs, b.monto_usd AS monto_usd 
		FROM 
			cobros_cliente AS a 
			JOIN cobros_cliente_detalle AS b ON b.cobros_cliente = a.id 
			LEFT OUTER JOIN salidas AS c ON c.id = a.id_documento 
		...
		SELECT 
			...
			a.monto_usd AS monto_usd 
		FROM recarga AS a JOIN cliente AS b ON b.id = a.cliente 
		WHERE fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND monto_usd > 0 */
		) AS aa 
...
while($row = mysqli_fetch_array($rs))
{
	if($xDoc != $row["metodo_pago"]) {
		if($xDoc != "") {
			...
			$pdf->Cell(30, 4, number_format($TotalBs, 2, ".", ","), "0", "0", 'R');
			$pdf->Cell(30, 4, number_format($TotalUsd, 2, ".", ","), "0", "0", 'R');
			$TotalBs = 0;
			$TotalUsd = 0;
			$items = 0;
			$pdf->SetFont('Arial', '', 8);
	...
	$pdf->Cell(10, 4);
	// $pdf->Cell(18, 4, $row["fecha"], 0, 0, 'L');
	$pdf->Cell(30, 4, substr($row["metodo_pago"], 0, 15), 0, 0, 'L');
	$pdf->Cell(30, 4, $row["tipo"], 0, 0, 'L');
	$pdf->Cell(20, 4, $row["doc"], 0, 0, 'C');
	$pdf->Cell(50, 4, substr($row["cliente"], 0, 45), 0, 0, 'L');
	$pdf->Cell(30, 4, number_format(abs($row["monto_bs"]), 2, ".", ","), 0, 0, 'R');
	$pdf->Cell(30, 4, number_format(abs($row["monto_usd"]), 2, ".", ","), 0, 0, 'R');

	$pdf->Ln();

	$TotalBs += abs($row["monto_bs"]);
	$TotalUsd += abs($row["monto_usd"]);
	if($row["monto_bs"] > 0) {
		$TotalGTBs += abs($row["monto_bs"]);
		$TotalGTUsd += abs($row["monto_usd"]);
	}
...
$pdf->Cell(30, 5, number_format($TotalGTBs, 2, ".", ","), 0, 0, 'R');
$pdf->Cell(30, 5, number_format($TotalGTUsd, 2, ".", ","), 0, 0, 'R');
...
$sql = "SELECT 
			...
			IF(a.metodo_pago='PF' OR a.metodo_pago='PC', (-1), 1)*a.monto_usd AS monto_usd, metodo_pago AS tipo_pago 
		FROM 
			recarga AS a 
			JOIN cliente AS b ON b.id = a.cliente  
		WHERE 
			a.fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND a.monto_usd > 0 ORDER BY a.metodo_pago;"; 
