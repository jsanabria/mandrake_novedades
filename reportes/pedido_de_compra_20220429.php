...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(30, 6, "UNIDA MED.", 1, 0, 'C');
...
while($row = mysqli_fetch_array($rs))
{
	...
	if(strlen($row["articulo"]) < 35) 
		$pdf->Cell(120, 5, $row["articulo"], 0, 0, 'L');
	else 
		$pdf->Cell(120, 5, substr($row["articulo"], 0, 55), 0, 0, 'L');
	$pdf->Cell(30, 5, $row["unidad_medida"], 0, 0, 'C');
	$pdf->Cell(20, 5, $row["cantidad"], 0, 0, 'R');

	if(strlen($row["articulo"]) >= 55) {
		$pdf->Ln();
		$pdf->Cell(30, 5);
		$pdf->MultiCell(130, 5, substr($row["articulo"], 55, strlen($row["articulo"])), 0, 'L');
	}
	else $pdf->Ln();
