...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->SetFont('Arial','',8);
	if(strlen($row["articulo"]) < 46) 
		$pdf->Cell(80, 5, $row["articulo"], 1, 0, 'L');
	else 
		$pdf->Cell(80, 5, substr($row["articulo"], 0, 46), 1, 0, 'L');
	$pdf->SetFont('Arial','B',8);
	...
	if(strlen($row["articulo"]) >= 46) {
		...
		$pdf->Cell(5, 5, "", "0", "0");
		$pdf->Cell(45, 5, "", "L", "0");
		$pdf->MultiCell(155, 5, substr($row["articulo"], 46, strlen($row["articulo"])), "R", 'L');
	}
	else $pdf->Ln();
