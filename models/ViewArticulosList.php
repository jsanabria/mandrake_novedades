...
class ViewArticulosList extends ViewArticulos
{
    ...
    // Table name
    public $TableName = 'view_articulos';
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["view_articulos"]) || get_class($GLOBALS["view_articulos"]) == PROJECT_NAMESPACE . "view_articulos") {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fview_articuloslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("view_articulos"));
                $doc->Text = @$content;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_cantidad_en_mano") && $CurrentForm->hasValue("o_cantidad_en_mano") && $this->cantidad_en_mano->CurrentValue != $this->cantidad_en_mano->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_ultimo_costo") && $CurrentForm->hasValue("o_ultimo_costo") && $this->ultimo_costo->CurrentValue != $this->ultimo_costo->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->codigo->clearErrorMessage();
        $this->fabricante->clearErrorMessage();
        $this->nombre->clearErrorMessage();
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fview_articuloslistsrch", $filters);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_articuloslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_articuloslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fview_articuloslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->cantidad_en_mano->CurrentValue = 0.00;
        $this->cantidad_en_mano->OldValue = $this->cantidad_en_mano->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->cantidad_en_mano->ViewValue = FormatNumber($this->cantidad_en_mano->ViewValue, $this->cantidad_en_mano->DefaultDecimalPrecision);
            ...
            $this->cantidad_en_mano->EditValue = HtmlEncode($this->cantidad_en_mano->CurrentValue);
            ...
            if (strval($this->cantidad_en_mano->EditValue) != "" && is_numeric($this->cantidad_en_mano->EditValue)) {
                $this->cantidad_en_mano->EditValue = FormatNumber($this->cantidad_en_mano->EditValue, -2, -1, -2, 0);
            }
            ...
            $this->cantidad_en_mano->EditValue = FormatNumber($this->cantidad_en_mano->EditValue, $this->cantidad_en_mano->DefaultDecimalPrecision);
            $this->cantidad_en_mano->ViewCustomAttributes = "";
            ...
            $this->ultimo_costo->EditAttrs["class"] = "form-control";
            $this->ultimo_costo->EditCustomAttributes = "";
            $this->ultimo_costo->EditValue = HtmlEncode($this->ultimo_costo->AdvancedSearch->SearchValue);
            $this->ultimo_costo->PlaceHolder = RemoveHtml($this->ultimo_costo->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->cantidad_en_mano->Required) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // cantidad_en_mano
        $this->cantidad_en_mano->setDbValueDef($rsnew, $this->cantidad_en_mano->CurrentValue, 0, strval($this->cantidad_en_mano->CurrentValue) == "");

        // ultimo_costo
        $this->ultimo_costo->setDbValueDef($rsnew, $this->ultimo_costo->CurrentValue, 0, strval($this->ultimo_costo->CurrentValue) == "");
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fview_articuloslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fview_articuloslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fview_articuloslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_view_articulos" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_view_articulos\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fview_articuloslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fview_articuloslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    {
        // Example:
        $sql = "SELECT id, nombre FROM tarifa WHERE patron = 'S';";
        $row = ExecuteRow($sql);
        $tarifa = $row["id"];
        $tarifa_nombre = $row["nombre"];

        // $header = '<a class="btn btn-primary" href="ConfirmPage?page=SyncTarifa&id=' . $tarifa . '">Agregar Articulos Nuevos Activos a la Tarifa ' . $tarifa_nombre . ' <span class="fas fa-sync"></span></a>';
    }
