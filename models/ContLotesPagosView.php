...
class ContLotesPagosView extends ContLotesPagos
{
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            ...
            $returnUrl = "ContLotesPagosList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cont_lotes_pagos')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=cont_lotes_pagos_detalle"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cont_lotes_pagos')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=cont_lotes_pagos_detalle"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    public function pageDataRendering(&$header)
    {
        // Example:
        //$header = "your header";
    	if($this->comprobante->CurrentValue == "N" or $this->comprobante->CurrentValue == "") {
    ...
    public function pageDataRendered(&$footer)
    {
        // Example:
        //$footer = "your footer";
    	$sql = "SELECT COUNT(id_documento) AS cantidad FROM cont_lotes_pagos_detalle WHERE cont_lotes_pago = " . $this->id->CurrentValue . ";";
    	$cantidad = ExecuteScalar($sql);
    	$footer = '<div class="alert alert-success" role="success">
    					Facturas asocialdas al lote ' . $cantidad . '
    				</div>';
    }
