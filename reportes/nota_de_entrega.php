...
$sql = "SELECT 
			id, date_format(fecha, '%d/%m/%Y') as fecha, cliente, nro_documento, tipo_documento, estatus, cliente, username, pago_premio 
...
$GLOBALS["pago_premio"] = $row["pago_premio"];
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$sql = "SELECT valor2 FROM parametro WHERE codigo = '048';";
		$rs = mysqli_query($link, $sql);
		$row = mysqli_fetch_array($rs);
		$cod_cia = $row["valor2"];
		...
		$this->SetFont('Arial','',8);
		$this->Cell(140, 5, "ESTATUS: " . $GLOBALS["estatus"] . " / " . "No. Doc.: $cod_cia-" . $GLOBALS["nro_documento"],0,0,'R');
	...
	function EndReport($id_invoice)
	{
		...
		$moneda = ($moneda == "USD" ? "USD" : $moneda); // Junior EURO
		...
		if(floatval($row["iva"]) > 0.00) {
				...
				$this->SetFont('Arial','',8);
				$this->Cell(25, 4, number_format(($GLOBALS["pago_premio"] == "S" ? 0 : $row["total"]), 2, ",", "."), 0, 0, 'R');
	...
	function BodyReport() 
	{
		...
		while($row = mysqli_fetch_array($rs))
		{
			...
			*/
			...
			$this->Cell(25, 4, number_format(($GLOBALS["pago_premio"] == "S" ? 0 : $row["precio_unidad"]), 2, ",", "."), 0, 0, 'R');
			$this->Cell(25, 4, number_format(($GLOBALS["pago_premio"] == "S" ? 0 : $row["precio"]), 2, ",", "."), 0, 0, 'R');
		...
		if($row = mysqli_fetch_array($rs)) $saldo_actual .= "(Bs: * USD " . number_format($row["saldo"], 2, ".", ",") . " *)"; // Junior EURO

		$sql = "SELECT id, saldo FROM recarga2 WHERE cliente = " . $GLOBALS["cliente"] . " ORDER BY id DESC LIMIT 0, 1;";

		$rs = mysqli_query($link, $sql);
		// if($row = mysqli_fetch_array($rs)) $saldo_actual = "\n(Saldo actual abonos: *** USD " . number_format($row["saldo"], 2, ".", ",") . " ***)";
		if($row = mysqli_fetch_array($rs)) $saldo_actual .= "(USD: * USD " . number_format($row["saldo"], 2, ".", ",") . " *)"; // Junior EURO
		...
		while($row = mysqli_fetch_array($rs))
		{
			...
			$this->Cell(20, 4, ($row["moneda"] == "USD" ? "USD" : $row["moneda"]) . " " . ($GLOBALS["pago_premio"] == "S" ? 0 : $row["monto_moneda"]), 0, 0, 'R');  // Junior EURO
			...
			if(trim($row3["nro_recibo"]) != "") {
				if($sw == false) {
					$this->SetFont('Arial','B',8);
					$recibo = str_pad($row3["nro_recibo"], 7, "0", STR_PAD_LEFT) . " / USD " . number_format($row3["monto_usd"], 2, ".", ",");
					$this->Cell(60, 4);
