	...
	if(intval($_GET['user']) == 365) {
		if(isset($_GET['app'])) {
			...
			case "canje_premios":
			    ...
			    // Consulta principal: Filtrando por pago_premio = 'S'
			    $sql = "SELECT 
			                hd.fecha, hd.moneda, hd.costo, hd.precio,  
			                1 AS documentos, 
			                1 AS articulos, 
			            ...
			            FROM 
			                ...
			                WHERE 
			                    ...
			                    AND IFNULL(a.pago_premio, 'N') = 'S' -- Condición modificada para canje de premios
			                    AND a.fecha BETWEEN '$fecha_inicio 00:00:00' AND '$fecha_fin 23:59:59' 
			                    AND a.estatus = 'PROCESADO' 
			                GROUP BY 
			                    DATE_FORMAT(a.fecha, '%Y-%m-%d'), a.moneda   
			    ...
			    $objCanjePremios = new stdClass();
			    $listaCanjePremios = [];

			    while($row = mysqli_fetch_array($rs)) {
			        $CanjePremios = new stdClass();

			        $CanjePremios->fecha = $row["fecha"];
			        $CanjePremios->moneda = $row["moneda"];
			        $CanjePremios->costo = $row["costo"];
			        $CanjePremios->precio = $row["precio"];

			        // Conteo de documentos para canje de premios
			        $sqlDoc = "SELECT COUNT(nro_documento) AS documentos 
			                   FROM salidas 
			                   WHERE tipo_documento = 'TDCNET'
			                     AND IFNULL(pago_premio, 'N') = 'S'
			                     AND DATE_FORMAT(fecha, '%Y-%m-%d') = '" . $row["fecha"] . "'  
			                     AND estatus = 'PROCESADO';"; 
			        $rs2 = mysqli_query($link, $sqlDoc);
			        $row2 = mysqli_fetch_array($rs2);
			        $CanjePremios->documentos = intval($row2["documentos"]);

			        // Conteo de artículos para canje de premios
			        $sqlArt = "SELECT ABS(SUM(y.cantidad_movimiento)) AS articulos 
			                   FROM salidas AS x 
			                   JOIN entradas_salidas AS y ON y.tipo_documento = x.tipo_documento AND y.id_documento = x.id 
			                   WHERE x.tipo_documento = 'TDCNET' 
			                     AND IFNULL(x.pago_premio, 'N') = 'S'
			                     AND DATE_FORMAT(x.fecha, '%Y-%m-%d') = '" . $row["fecha"] . "'  
			                     AND x.estatus = 'PROCESADO';";
			        $rs2 = mysqli_query($link, $sqlArt);
			        $row2 = mysqli_fetch_array($rs2);
			        $CanjePremios->articulos = intval($row2["articulos"]);

			        // Obtención de tasa (se mantiene igual ya que depende de la fecha)
			        $sqlTasa = "SELECT bb.tasa_usd AS tasa  
			                    FROM cobros_cliente AS aa 
			                    JOIN cobros_cliente_detalle AS bb ON bb.cobros_cliente = aa.id 
			                    WHERE DATE_FORMAT(aa.fecha, '%Y-%m-%d') = '" . $row["fecha"] . "' 
			                    UNION SELECT aa.tasa_usd AS tasa  
			                    FROM recarga AS aa 
			                    WHERE DATE_FORMAT(aa.fecha, '%Y-%m-%d') = '" . $row["fecha"] . "' LIMIT 0,1;";
			        $rs2 = mysqli_query($link, $sqlTasa);
			        $row2 = mysqli_fetch_array($rs2);
			        $CanjePremios->tasa = floatval($row2["tasa"]);
			        
			        $CanjePremios->tienda = $codigo;
			        $CanjePremios->id = NULL;

			        $listaCanjePremios[] = $CanjePremios;
			    }

			    mysqli_close($link);

			    $objCanjePremios->listaCanjePremios = $listaCanjePremios;
			    echo json_encode($objCanjePremios, JSON_UNESCAPED_UNICODE);
