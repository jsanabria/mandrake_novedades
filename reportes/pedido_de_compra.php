...
$sql = "SELECT 
			id, date_format(fecha, '%d/%m/%Y') as fecha, proveedor, nro_documento, tipo_documento, estatus, descuento  
...
$GLOBALS["descuento"] = $row["descuento"];
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->MultiCell(160, 5, "$direccion_proveedor. $ciudad_proveedor", '0', 'L');
		...
		$this->SetFont('Arial','B',8);
		$this->Cell(10, 4);
		$this->Cell(70, 4,"DESCUENTO GLOBAL: " . $GLOBALS["descuento"],'0',0,'L');
		$this->SetFont('Arial','',8);
		*/
		...
		$this->Cell(20, 6, "PROFIT", 1, 0, 'C');
		$this->Cell(65, 6, "ARTICULO", 1, 0, 'L');
		$this->Cell(10, 6, "CANT", 1, 0, 'R');
		$this->Cell(20, 6, "COST FUL", 1, 0, 'R');
		$this->Cell(15, 6, "DESC", 1, 0, 'R');
...
$sql = "SELECT 
			...
			WHERE 
				bb.id = a.id_documento AND aa.articulo = a.articulo) AS codigo2,  
			...
			a.costo_unidad, a.descuento, a.precio_unidad_sin_desc, 
			a.costo, a.lote, date_format(a.fecha_vencimiento, '%d/%m/%Y') AS vencimiento, b.codigo AS profit  
		FROM 
...
while($row = mysqli_fetch_array($rs))
{
	$pdf->SetFont('Arial', '', 8);
	$pdf->Cell(10, 4);
	$pdf->Cell(20, 4, substr($row["codigo"], 0, 11), 0, 0, 'L');
	$pdf->Cell(20, 4, substr($row["profit"], 0, 11), 0, 0, 'L');
	if(strlen($row["articulo"]) < 35) 
		$pdf->Cell(65, 4, $row["articulo"], 0, 0, 'L');
	else 
		$pdf->Cell(65, 4, substr($row["articulo"], 0, 35), 0, 0, 'L');
	$pdf->Cell(10, 4, intval($row["cantidad"]), 0, 0, 'R');
	$pdf->Cell(20, 4, number_format($row["precio_unidad_sin_desc"], 2, ",","."), 0, 0, 'R');
	$pdf->Cell(15, 4, number_format($row["descuento"], 2, ",","."), 0, 0, 'R');
	...
	if(strlen($row["articulo"]) >= 35) {
		$pdf->Ln();
		$pdf->Cell(50, 4);
		$pdf->MultiCell(130, 4, substr($row["articulo"], 35, strlen($row["articulo"])), 0, 'L');
	}
	else $pdf->Ln();
